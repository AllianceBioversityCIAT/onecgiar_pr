...
**Directory boundaries** — use these to judge task independence and parallel eligibility:

| Boundary | Path | Independence rule |
|---|---|---|
| Backend feature module | `onecgiar-pr-server/src/api/<feature>/` (module/controller/service/DTO/entities) | Two tasks in different feature folders are parallel-safe unless both add a **migration** |
| Backend migrations | `onecgiar-pr-server/src/migrations/` | **Never parallel** — one migration-bearing task at a time (`migration:check:ci`) |
| Backend shared | `onecgiar-pr-server/src/shared/` (handlers, constants, enums) | Serialise any task touching it |
| Bilateral / platform-report payloads | `src/api/bilateral*`, `src/api/platform-report*`, doc `onecgiar-pr-server/docs/bilateral-result-summaries.en.md` | Serialise; every change updates the doc's change log |
| Frontend page module | `onecgiar-pr-client/src/app/pages/<domain>/` (results, quality-assurance, ipsr-framework, type-one-report, …) | Parallel-safe across domains |
| Frontend shared | `onecgiar-pr-client/src/app/shared/` (services/api, components, guards, interceptors) and `src/styles/` (tokens) | Serialise |
| Baseline docs / guides | `docs/`, `CLAUDE.md`, `AGENTS.md`, `.agents/` | Shared-file write discipline applies |

**Exemplar briefing:** name the closest existing module as the pattern — e.g. for a new server feature `src/api/results/…` siblings; for a result-detail section `pages/results/pages/result-detail/pages/rd-*`.

**Verification commands to hand Implementers/Testers:** see `docs/infrastructure.md` §6 (agent-lean variants). Never run a build/E2E/benchmark yourself while a delegated agent is active.

**Skills to assign:** from `## Skill Map` in `AGENTS.md` — `nestjs-expert` (server), `angular-developer` (client), `api-design-principles` / `error-handling-patterns` (API surface), `aws-serverless` (Lambda handler/bundling), `tdd` for logic-heavy tasks.
