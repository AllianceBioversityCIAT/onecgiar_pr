...
- [x] 1.1 Extend `bilateral-page-header` with result code, result type, a `W3/Bilateral` funding tag
      and a status badge. Today it renders only the centre breadcrumb, title, acronym and a CTA.
- [ ] 1.2 🔴 BLOCKED (P2-3437 item 6 — the detail response carries no status) — Expose a read-only signal derived from `status_id`: editable only in Editing. The value is
...
- [ ] 1.3 BLOCKED by 1.2 — Thread it through the five sections. ⚠️ `section-geography.component.html:138` and `:272`
...
- [ ] 1.5 Partly done (identity strip tested; badge blocked) — Tests: badge and identity strip per status; every section read-only outside Editing;
      mutation check on the read-only derivation.
- [x] 1.6 Spec collision to raise, not resolve: P2-3352 says Primary contributing science program
...
- [x] 2.1 Read `section-geography.component.html` in full (555 lines) before editing anything.
- [x] 2.2 Compare field by field against the story's scope table (Global 1, Regional 2, Country 3,
      Sub-national 5, To-be-determined 50) and the per-scope region/country rules.
- [x] 2.3 Extra-geographic-scope question present for every scope except Global and To-be-determined.
- [x] 2.4 Confirm the MDS checklist this section publishes still matches what it renders.
- [x] 2.5 Tests per scope option, including the two that must NOT show the extra question.
- [x] 2.6 Do **not** replace it with `app-geoscope-management`. Record the divergence on the ticket.

## 3. P2-3375 — Evidence parity

- [x] 3.1 Read `section-evidence.component.html` in full (430 lines) and its own model first.
- [x] 3.2 (audit done, 6 comply / 3 gaps — see P2-3440) Walk the story's rule list: six-item cap · Link and Upload file · public/private ·
