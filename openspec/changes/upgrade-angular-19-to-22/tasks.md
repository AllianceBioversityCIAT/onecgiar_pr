...
- [ ] 0.1 Confirm isolated worktree `.claude/worktrees/angular-upgrade-19-22` (branch `angular-upgrade-19-22` off `staging`), tree clean
- [ ] 0.2 `cd onecgiar-pr-client && npm ci` — install baseline (Angular 19) node_modules in the worktree
- [ ] 0.3 Baseline gate — record that `npm run build:dev`, `npm run test`, `npm run test:ct` are green on Angular 19 BEFORE upgrading (so regressions are attributable)
- [ ] 0.4 Check local Node version vs Angular 20/21/22 engine floors; note if a bump is needed

## 1. Major 19 → 20 (frontend)

- [ ] 1.1 `ng update @angular/core@20 @angular/cli@20 @angular/cdk@20` — let framework + CDK migration schematics run
- [ ] 1.2 Bump lockstep deps to major 20: `primeng@20`, `@primeng/themes@20`, `@ncstate/sat-popover@15`, `@angular-eslint/*@20`, build tooling (`@angular-devkit/build-angular`, `@angular/build`, `@angular/compiler-cli`); `npm install` resolves with no ERESOLVE
- [ ] 1.3 Fix compiler/template errors from Angular 20 + PrimeNG 20 API drift (minimal, no behaviour change)
...
- [ ] 1.6 Commit `chore(deps): upgrade Angular 19→20 (PrimeNG/CDK/sat-popover lockstep)`; log codemod + API changes in the QA doc
