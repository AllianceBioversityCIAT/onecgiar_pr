...
- [ ] 5.1 `cd onecgiar-pr-client && npm start`, sign in, open a **P25 innovation use** result.
- [ ] 5.2 Section 2: answer **Yes**, select two linked results, save. Section 4: confirm the question is gone. Save Section 4. Return to Section 2 → the answer is still Yes and both linked results are still there.
- [ ] 5.3 Repeat with **No** in Section 2: save Section 4, return to Section 2 → still No.
...
- [ ] 5.5 Regression: open a **P22** innovation use result and save the section — no error, no visible change from before.
- [ ] 5.6 Regression: open the **IPSR** innovation use pathway step 1 → renders exactly as before.
- [ ] 5.7 Confirm persistence with a read-only check against the test backend, e.g. `curl -s -H "auth: $TOKEN" "https://prtest-back.ciat.cgiar.org/..."` for the same result id, and compare `has_innovation_link` / linked results before and after a Section 4 save.
- [ ] 5.8 Capture before/after screenshots into `onecgiar_pr/.local-screenshots/` (gitignored) as `p2-3199-*.png` for the Jira update.

## 6. Hand-offs (no code here)

- [ ] 6.1 Comment on [P2-3199](https://cgiarmel.atlassian.net/browse/P2-3199) with what was done, how to verify, branch and commits.
- [ ] 6.2 Report to the backend team, on the same ticket, that `PATCH /results-framework-reporting/innovation-use` deletes linked results when `has_innovation_link` is absent (`if (!has_innovation_link) → createForInnovationUse(resultId, [], user)`), and that the endpoint should ignore fields that are not present. Include the file/line evidence. **Do not modify server code.**
- [ ] 6.3 Notify Santiago Sánchez on Slack when the change is deployed to testing, and remind him the green check / P2-3191 stays with him and Juan David.
