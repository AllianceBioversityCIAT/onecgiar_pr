...
| **Env templates** | Backend `.env` keys listed in `README.md` → *Environment*; includes `CGSPACE_DISCOVERY_URL`, `MELSPACE_DISCOVERY_URL` and `WORLDFISH_DISCOVERY_URL` (each required for its repository, no in-code fallback; recommended values `https://cgspace.cgiar.org/server/api`, `https://repo.mel.cgiar.org/server/api` and `https://digitalarchive.worldfishcenter.org/server/api`; a missing one degrades only that repository to `unconfigured`); client config in `onecgiar-pr-client/src/environments/environment.ts`; Cypress env example at `onecgiar-pr-client/cypress.env.js.example`. |
