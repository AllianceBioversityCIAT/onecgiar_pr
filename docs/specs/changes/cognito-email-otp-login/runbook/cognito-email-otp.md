...
| 2 | PRMS user missing or inactive | `AuthService.startOtp` looks the user up with `findOne({ where: { email, active: true } })`; a miss **or** an inactive row both fall through the same branch and return the identical neutral `200` (decoy session, no mail), logging `auth.otp.start { domain, outcome: 'denied_user' }`. **There is only one outcome name for this case** — do not grep for `unknown_user` or `inactive`, they don't exist in the code (`onecgiar-pr-server/src/auth/auth.service.ts`, `startOtp`). | Confirm the user exists in PRMS and `active = true`. If not, create/reactivate the account first — the Center path never creates PRMS or Cognito users itself. |
| 3 | **PRMS user exists but is not in Cognito** | Cognito's `PreventUserExistenceErrors=ENABLED` returns a *simulated* `EMAIL_OTP` challenge for a username that has no Cognito account. The microservice call still returns a session, so PRMS logs `auth.otp.start { domain, outcome: 'sent' }` and the user reaches the code step exactly as a real user would — **but no mail is ever sent**, because there is no real Cognito account behind the simulated challenge. | Provision the user in Cognito via the microservice's `/auth/register`. If the user's email domain is in `PASSWORDLESS_DOMAINS`, `OTP-T-10`'s adjusted provisioning lands them `CONFIRMED` directly, so their next `start` gets a real, deliverable code. |
