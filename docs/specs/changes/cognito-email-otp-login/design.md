...
- **(m) Unknown-user destination mask in `CreateAuthChallenge`** (seen 2026-09-11 post-rollback sanity): for `userNotFound` the trigger returns a random mask (`b***@g***`) instead of one derived from the typed username (`s***@i***`), so at the microservice boundary the mask does not match the input — derive the mask from `event.userName` for unknown users. Not client-visible today (PRMS answers unknown users with a decoy before calling the microservice); fix before PROD.
