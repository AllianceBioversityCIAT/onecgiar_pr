...
- **Rate limiting:** a **named throttler** — `ThrottlerModule.forRoot([{ ttl: 60000, limit: 100 }, { name: 'otp', ttl: 900000, limit: 10 }])` — with the two routes decorated `@SkipThrottle({ default: true })` + `@Throttle({ otp: { limit: 5 | 10, ttl: 900000 } })`, so the app-global `ThrottlerExcludeBilateralGuard` (`APP_GUARD`, default throttler, `req.ip` tracker) **skips** them; `OtpThrottlerGuard` (subclass, `@UseGuards`) handles only the `otp` throttler: `getTracker` → normalised body email capped at 254 chars and required to contain `@` (fallback first `x-forwarded-for` hop, then `req.ip`); `throwThrottlingException(context, detail)` emits `auth.otp.<start|verify> { domain, outcome: 'rate_limited', durationMs }` and throws the neutral 429 envelope. Storage is the module default (**in-memory, per instance**) — accepted for ≤ 50 users on a single Lambda/container, Cognito's own attempt/expiry limits are the real backstop; horizontal scaling requires a shared store (§13). One composition test boots `APP_GUARD` + the route guard together and proves the 6th call returns `OTP_RATE_LIMITED` while a different email from the same IP passes.
