...
| Budget | **9 tasks (+1 conditional) · ~1,250 LOC incl. tests (microservice ~240, PRMS server ~420, PRMS client ~400, CT ~70, runbook/docs ~60, conditional provisioning ~60) · ≤ 1 review round per task; tripwire > 12 tasks or > 1,500 LOC → stop and escalate** |
...
| Response | `{ response: { domains: string[] }, message, status }` | `200 { response: { sent: true, session: string, destination: string }, message: 'If this account exists, a code has been sent.', status: 200 }` — **byte-identical shape for known and unknown users**: unknown/inactive users get a server-signed **decoy session** (`otp:<base64url(HMAC-SHA256(JWT_SKEY, email|nonce|exp))>`, padded to the length class of a Cognito session) and a masked destination derived from the submitted email (`j***@icrisat.org`) | `200` exactly the `/login/custom` success shape: `response: { valid: true, token, user: {…}, auth_tokens }` (the object `createSuccessfulLoginResponse` builds) |
| Errors | — | `400 OTP_DOMAIN_NOT_ALLOWED` (domain outside list) · `429 OTP_RATE_LIMITED` neutral copy (custom body, not the raw `ThrottlerException`) · `503 OTP_UPSTREAM_UNAVAILABLE` (microservice/Cognito down) | `401 OTP_CODE_MISMATCH` · `401 OTP_CODE_EXPIRED` · `401 OTP_ATTEMPTS_EXCEEDED` (client must restart) · `401 OTP_NOT_AUTHORIZED` (neutral — decoy session, unknown user, bad or expired session, or a Cognito challenge the flow does not support) · `403 needsRoles` (same as `/login/custom` when the PRMS user has no roles) · `429` · `503` |
...
| Response | `{ challengeName: 'EMAIL_OTP', session, codeDeliveryDestination? }`; if Cognito answers `SELECT_CHALLENGE` (listing factors) the service answers it with a second `RespondToAuthChallenge { ClientId, ChallengeName: 'SELECT_CHALLENGE', Session: <session from InitiateAuth>, ChallengeResponses: { USERNAME, ANSWER: 'EMAIL_OTP', SECRET_HASH } }` and returns the **new** `Session` from that reply (the one PRMS must replay on verify) — the spike (`OTP-T-1`) pins which shape the pool returns. Any other challenge (`NEW_PASSWORD_REQUIRED`, MFA…) → `CHALLENGE_NOT_SUPPORTED` | `{ tokens: { accessToken, idToken, refreshToken, expiresIn, tokenType } }` — identical to `authenticateWithCustomPassword`; a challenge instead of tokens → `CHALLENGE_NOT_SUPPORTED` |
| Errors (stable codes) | `NOT_AUTHORIZED`, `CHALLENGE_NOT_SUPPORTED`, `UPSTREAM_ERROR` (502). **Unknown users do not error**: with `PreventUserExistenceErrors=ENABLED` Cognito returns a *simulated* `EMAIL_OTP` challenge with a dummy session and masked destination (JA-10/JB-11) — the spike confirms; PRMS never reaches this for unknown PRMS users (decoy path), only for PRMS users missing in Cognito | `CODE_MISMATCH` (`CodeMismatchException`), `CODE_EXPIRED` (`ExpiredCodeException`), `ATTEMPTS_EXCEEDED` (`NotAuthorizedException` with the attempts message, or `TooManyFailedAttemptsException`), `NOT_AUTHORIZED` (incl. invalid/dummy session), `CHALLENGE_NOT_SUPPORTED`, `UPSTREAM_ERROR` |
| Env | reuses `COGNITO_CLIENT_ID`, `COGNITO_CLIENT_SECRET`, `COGNITO_USER_POOL_URL`; `OTP-OQ-6` verifies the client id is the PRMS-Reporting client |
...
- **startOtp:** normalise → domain check (400, `denied_domain`) → **per-email counter (429 `OTP_RATE_LIMITED`, `rate_limited`) — before any lookup so known and unknown emails behave identically (JA-4)** → local user lookup `findOne({ where: { email, active: true }, relations: ['obj_role_by_user'] })`; miss/inactive → neutral 200 with a **decoy session** and a destination masked from the submitted email, `outcome: denied_user`, no microservice call → otherwise microservice `startEmailOtp` → 200 with the Cognito `session` and its masked `destination`. Upstream failure → 503 `OTP_UPSTREAM_UNAVAILABLE`, `outcome: upstream_error`. Never creates users, never calls `/auth/register`.
- **verifyOtp:** normalise → counter (429) → decoy check (`session` starts with `otp:` and its HMAC verifies → 401 `OTP_NOT_AUTHORIZED`, `outcome: not_authorized`, no microservice call) → local user lookup **with `relations: ['obj_role_by_user']`** (miss → 401 `OTP_NOT_AUTHORIZED`) → microservice `verifyEmailOtp` → map error codes (`CHALLENGE_NOT_SUPPORTED` → 401 `OTP_NOT_AUTHORIZED` with support copy) → on tokens: `updateLastLoginUserByEmail` then **`createSuccessfulLoginResponse(user, tokens)`** exactly as `singIn` does after the password flow (same method, same arguments, same relation-loaded `user`, no new branch inside it — including its `403 needsRoles` guard, JA-2/JB-2).
- **Rate limiting:** the `@nestjs/throttler` guard with an OTP-specific subclass (`getTracker` → normalised email from the body, fallback first `x-forwarded-for` hop, then `req.ip`; `throwThrottlingException` → the neutral 429 envelope). Storage is the module default (**in-memory, per instance**) — accepted for ≤ 50 users on a single Lambda/container, Cognito's own attempt/expiry limits are the real backstop; horizontal scaling requires a shared store (§13).
...
- **Decision:** the domain is public knowledge (it is on the button), so a foreign domain gets a helpful 400; existence is never revealed — unknown/inactive users receive a **byte-identical** 200 with a server-signed decoy session and a masked destination, the rate limiter counts before the lookup, and their verify fails with the same `401 OTP_NOT_AUTHORIZED` a wrong code produces (JA-1/JB-1, JA-4).
...
- **Decision:** one guard subclass keyed on the normalised email (fallback `x-forwarded-for`), applied before any user lookup so 429 behaviour is identical for known and unknown emails, with a neutral custom 429 body; Cognito's attempt/expiry limits are the real backstop.
...
- **`FORCE_CHANGE_PASSWORD` contingency (`OTP-R-13`, JA-5/JB-7):** if the spike shows Cognito refuses `EMAIL_OTP` for users still in `FORCE_CHANGE_PASSWORD`, the adjustment is in the **provisioning path, not the login path**: the AUTH microservice's `/auth/register` (`createUser`) sets a permanent random password via `AdminSetUserPassword --permanent` right after `AdminCreateUser` (no welcome-password email for center domains) so the user lands `CONFIRMED` with `email_verified=true`; PRMS admin UI unchanged. Reserved as conditional task `OTP-T-10` (~60 LOC, microservice) — executed only if the spike says so.
