  ...
  └─ email step: client validates syntax + domain ∈ domains
       ...
       └─ code step: POST auth/login/otp/verify { email, code, session }   (@Throttle 10/15min)
            └─ AuthService.verifyOtp
                 ├─ decoy session (HMAC valid, unexpired) → 401 OTP_CODE_MISMATCH without any MS call; expired decoy → 401 OTP_NOT_AUTHORIZED
...
| Throttle | global default | route `@SkipThrottle()` (global guard ignores it) + `@UseGuards(OtpThrottlerGuard)` with guard-owned limits `start 5 / 15 min` under an `'otp'` storage namespace; `getTracker` returns the **normalised email** (≤ 254 chars, must contain `@`; fallback: first `x-forwarded-for` hop, then `req.ip`), so the bucket is per account, not per proxy; `throwThrottlingException` logs `rate_limited` and returns the neutral 429 body below. Counted **before** the user lookup — known and unknown emails hit 429 identically (JA-4). **Both limits are per-instance** (default in-memory storage) — see `OTP-DD-6` | same guard, `verify 10 / 15 min` |
...
| Telemetry | — | `auth.otp.start { domain, outcome: sent \| denied_domain \| denied_user \| rate_limited \| upstream_error \| internal_error, durationMs }` — `denied_domain` is emitted on the 400 path, `rate_limited` on the 429 path | `auth.otp.verify { domain, outcome: ok \| mismatch \| expired \| attempts_exceeded \| not_authorized \| upstream_error \| internal_error, durationMs }` |

Unknown/inactive users receive the same `200` body with a decoy session; the client renders the code step; their verify fails with `401 OTP_CODE_MISMATCH` ("Code incorrect") **without** a microservice call — the same answer a wrong code on a real session gets. A known-but-not-in-Cognito user also reaches the code step (Cognito simulates the challenge under `PreventUserExistenceErrors=ENABLED`) and never receives mail — the support runbook covers it (§9). Wire format stays the project envelope (`ResponseInterceptor`).
...
- New standalone `CenterOtpPanelComponent` (`pages/login/components/center-otp-panel/`), inputs `domains`, outputs none (it calls `CognitoService`). Internal state machine (signals): `step: 'email' | 'code'`, `email`, `session`, `destination`, `code`, `busy`, `error: null | 'domain' | 'mismatch' | 'expired' | 'attempts' | 'upstream' | 'rate'`, `resendCooldown` seconds (30 s; design name `resendAt`), panel-local `'format'` error key. Actions: `sendCode()`, `verify()`, `resend()`, `back()` (code → email only). The email-step Cancel lives on the login page: `LoginComponent` renders `data-test="otp-center-cancel"` next to the open panel (`OTP-T-7`), because the panel has no outputs.
...
- Copy (English, inline like the rest of `pages/login`): button "Continue with your Center account"; helper "For CGIAR centers outside the CGIAR directory · {{ centers }}" (middot separator as in `mockup/login-target.html`; labels joined with " · ") where `centers` is derived from domains via a small label map (`icrisat.org → ICRISAT`, `cifor-icraf.org → CIFOR-ICRAF`, fallback = domain); email placeholder `name@icrisat.org`; neutral sent copy "If this account exists, we sent a code to {{ destination || 'your email' }}"; errors "That email domain is not enabled for this option — use your CGIAR account or the external-user option, or contact PRMSTechSupport@cgiar.org", "Code incorrect. Try again.", "Code expired — request a new one.", "Too many attempts — request a new code.", "We could not reach the sign-in service. Try again in a minute or contact support.", "Too many requests — wait a few minutes."; client-authored strings of record (T-6/T-7): malformed code → "Enter the code from your email.", `needsRoles` and `unknown` fallbacks as shipped in `OTP_ERROR_COPY` (server `message` shown for unmapped codes when present)
...
| `CenterOtpPanelComponent` (new, standalone) | Email step and code step, state machine §5.3, inline errors, resend cooldown, back link; `aria-live="polite"` region for status/error text; `data-test` hooks `otp-email`, `otp-send`, `otp-code`, `otp-verify`, `otp-resend`, `otp-back`, `otp-status`; `otp-center-button`, `otp-center-helper` and `otp-center-cancel` belong to `LoginComponent`. |
...
- **Residual decoy fingerprints (T-5 lens B, accepted for TEST — fix before PROD if cheap):** (a) the `exp` segment is 13 ASCII digits at a fixed offset, a deterministic decoy-vs-real classifier from one `start` response — fix: carry `exp` as base64url of its byte form (~11 chars) and parse it back on verify; (b) jitter band 1,400–1,700 is wider than the observed real class (1,543 / 1,587) — narrow once more real sessions are sampled; (c) a real account eventually hits `OTP_ATTEMPTS_EXCEEDED` while a decoy answers `OTP_CODE_MISMATCH` for all attempts — accepted (no in-session lockout was observed on real sessions either, `OTP-OQ-7`).
- With `JWT_SKEY` unset the decoy key is a per-process random secret, so a decoy minted on one instance verifies as "real" on another and degrades to the microservice path (`OTP_NOT_AUTHORIZED`) — only in environments missing a required env var; recorded (T-5 lens A).
...
- **(l) Residual decoy statistics after the keyed mask (`OTP-T-14` step 0, 2026-09-11):** the trailing base64url char of each 8/16/32-byte segment can only take 16/16/4 alphabet values (unused low bits), so a decoy passes a 3-offset charset test a uniform random session passes with p ≈ 1/256 — fill the unused bits with randomness on encode (decoder ignores them); add a domain-separation byte between the exp-mask HMAC and the decoy-auth HMAC (same `_otpDecoyKey`). Cheap, before PROD. **Done 2026-09-12** (PRMS `998737f14`: `encodeOtpSegment` randomises the unused low bits of each segment's last char — 8 B→2 bits, 16 B→4 bits, 32 B→2 bits; tags `otp-decoy-auth\0` / `otp-decoy-mask\0`; Reviewer PASS, SUF-not-EUF note recorded).
- **(m) Unknown-user destination mask in `CreateAuthChallenge`** (seen 2026-09-11 post-rollback sanity): for `userNotFound` the trigger returns a random mask (`b***@g***`) instead of one derived from the typed username (`s***@i***`), so at the microservice boundary the mask does not match the input — **corrected 2026-09-12:** for `userNotFound` Cognito substitutes `event.userName`, so the trigger cannot match; the fix is in the AUTH microservice — `codeDeliveryDestination` derived locally from the request username, Cognito's value ignored. Not client-visible today (PRMS answers unknown users with a decoy before calling the microservice). **Done 2026-09-12** (microservice `5a758fd`, PR #42): live check — known/unknown same keys, mask and session length.
...
| 10 | PRMS server → client | `401 OTP_CODE_MISMATCH` **now carries the rotated `session`**; the client stores it before the retry (`OTP-T-13`). **Decoy mismatch mints a fresh decoy session too** (same `exp` as the original decoy, same length-jitter class) so real and decoy mismatch bodies keep identical key sets and copy (`OTP-R-4`, `OTP-DD-3` — the presence of the field is itself an oracle; corrected after the T-13 Reviewer, 2026-09-11). |
...
| `verify` | parse + HMAC (`timingSafeEqual`) → not a decoy-shaped session → `OTP_NOT_AUTHORIZED`; expired (real or decoy — the envelope carries the same `exp` as the row, so both answer alike) → `OTP_NOT_AUTHORIZED` (**amended 2026-09-12 after the T-16 review**: no post-expiry real-vs-decoy oracle; `OTP_CODE_EXPIRED` is no longer produced on this path); lookup row by `nonce`: **no row → decoy path** (mismatch with rotated decoy, unchanged); row `consumed_at` set → `OTP_NOT_AUTHORIZED`; `attempts >= 3` → `OTP_ATTEMPTS_EXCEEDED`; code HMAC mismatch → atomic conditional `attempts + 1 WHERE attempts < 3` (0 rows → `OTP_ATTEMPTS_EXCEEDED`), `401 OTP_CODE_MISMATCH` + rotated session (same nonce, fresh tails/filler); match → atomic `consumed_at = now WHERE consumed_at IS NULL` (0 rows → `OTP_NOT_AUTHORIZED`, no double-spend), T-15 auto-provision, `createSuccessfulLoginResponse(user, null)` (no Cognito tokens; `auth_tokens` omitted/null — unused downstream) |
