...
| `OTP-AC-5` | Email `nobody@icrisat.org` (not a PRMS user) | **Send code** | `200` body **byte-identical in shape** to AC-4 (decoy `session`, masked destination); no microservice call; `auth.otp.start outcome=denied_user`; the UI moves to the code step; any code then yields `401 OTP_NOT_AUTHORIZED` |
...
| `OTP-AC-12` | Cognito user in `FORCE_CHANGE_PASSWORD` with verified email | Center path | Sign-in succeeds (or the spike records the refusal and the provisioning adjustment) |
