...
### `UG-T-9` — Author guide content (intro + 6 sections) — IN PROGRESS (`[~]`, attempt 1 FAIL, attempt 2 dispatched)
...
- **Attempt 2:** dispatched with both FAIL issues verbatim + adopted advisories + glossary delta, effort high.

---

### `UG-T-3` — `routes.config.json` · `UG-T-7` — `capture.ts` — IN PROGRESS (`[~]`, attempt 1 FAIL, attempt 2 dispatched)
...
- **Attempt 2:** dispatched with all four FAIL issues verbatim, effort high.
...
8 of 16 tasks complete (`UG-T-1`, `UG-T-2`, `UG-T-4`, `UG-T-5`, `UG-T-6`, `UG-T-8`, `UG-T-10`, `UG-T-11`); `UG-T-5`/`UG-T-6` reviewed PASS on code, `[~]` until `UG-T-7`'s live run closes their visual/live DoD items. In flight: `UG-T-3`+`UG-T-7` attempt 2 (Implementer); `UG-T-9` attempt 2 (Implementer, includes glossary loop-back). Next eligible: `UG-T-2` (Verify environment and seed data) and `UG-T-8` (Resolve sign-in URL reference) — `UG-T-8` is now effectively pre-resolved by the user's decision (no URL, generic phrasing) and only needs a one-line confirmation note when its turn comes. `UG-T-2` is blocked pending the `TEST_TOKEN` value.
