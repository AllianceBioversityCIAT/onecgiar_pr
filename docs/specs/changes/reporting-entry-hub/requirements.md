...
| `REH-AC-4` | REH-AC-3 state | The user types `1368` in the search | Only rows whose code or name contains `1368` remain, across all centers; centers with a match render expanded, centers without a match render collapsed while the query is active (their pre-search state is restored on clear — REH-R-3.4); counter shows `1 / 61`. **BUT** clearing the search restores the 3-row slices and collapse states. |
