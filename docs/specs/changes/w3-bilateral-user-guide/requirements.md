...
- The worktree's own client, running per `docs/infrastructure.md` §6, against a database holding the data shape in `BG-OQ-1`.
...
| `BG-OQ-1` | **PARTLY RESOLVED (operator, 2026-09-21).** Centre and project confirmed: **`Bioversity (Alliance)` + `B-A1368`** (multi-SP, SP01 80% / SP13 20%). **Environment and reporter JWT remain open** — the operator chose to start execution without them. | The Centre/project choice was recommendable from the supplied screenshot and is now fixed. Which environment to shoot and the credential for it are not derivable from the repository. Owned by `BG-T-8`; **blocks `BG-T-7` onward only** — `BG-T-1`…`BG-T-6` need no environment and are cleared to run. |

**Gating status:** `BG-OQ-1`'s remaining half (environment + credential) blocks **`BG-T-7` onward**; `BG-T-1`…`BG-T-6` are unblocked. `BG-OQ-2` is resolved to *investigate, then fall back to prose* (operator, 2026-09-21) — `BG-T-11` searches for an already-assessed result and takes option (c) if none exists. The other four are closed.
