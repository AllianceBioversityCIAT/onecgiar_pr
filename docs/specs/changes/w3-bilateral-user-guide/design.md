...
|---|---|---|
| Declarative per-route `steps` (click / waitFor / press) run between `goto` and annotation | The drawer and the six editor sections have no URL | `BG-R-14`, `BG-R-3`, `BG-R-4` |
...
`Step` is a closed, three-variant union — deliberately not a script:
...
**Count:** 13 rows — **12 verified**, **1 `UNVERIFIED`** (Impact: **0 High**, **1 Low**). Post-judgment: `P-7` promoted to verified (a judge reached its primary source), `P-1` corrected, and `P-11`–`P-13` added for premises the first draft depended on without a row.
...
| `P-10` | Adding optional `steps`/`bounds`/`fill` to `RouteConfig` has exactly one consumer — the same copy's `capture.ts`. | `consumer` | `UNVERIFIED — confirm at source before relying on it` — the claim is about a copy that does not exist yet, so it cannot be verified now. Supporting sweep over the **archived** copy, as run: `grep -rn "RouteConfig\|routes.config" --include='*.ts' --include='*.json' .` → hits confined to `.../tooling/src/capture.ts` (declaration `:70`, `loadRoutes()` `:125-130`) and `routes.config.json` itself; no test file, E2E suite, or other app reads either. | — | A second consumer would have to be updated in lockstep. Impact **Low** | `BG-T-4` re-runs the sweep over the new copy once it exists |
