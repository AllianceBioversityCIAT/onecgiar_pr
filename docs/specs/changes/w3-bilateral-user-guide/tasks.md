...
### `BG-T-9` — Route config: the editor *(captures 8–15)*  `[x]`
...
- ⚠️ **CARRIED FROM `BG-T-9` (2026-09-21).** A reproducible **client-side timing race** was observed across three identical read-only runs: some sections rendered complete on one run and showed a transient "field(s) missing" state on another, with heading text stable throughout — so no caption gate catches it. **Never assert a specific field value, partner name, or completion state as guaranteed by a capture.** Describe what the control does, not what one screenshot happened to contain. Defect class **D9**, no automated gate.
...
- ⚠️ **CARRIED FROM `BG-T-9` (2026-09-21) — two obligations, both load-bearing.**
  1. **`BG-R-6` exception condition (b), three new cases.** Three editor callouts ship **ring-only** and their geometry is recorded in `execution.md`: the **section rail** (`bilateral-sections-rail` — a 240px full-height column with zero gutter, so any chip lands on the adjacent card), the **footer position indicator** (`bilateral-footer-position` — a labelled version was shipped in round 1 and its connector visibly crossed the "Section complete" badge, obscuring text; caught by viewing the PNG), and the **submit note** (`bilateral-rail-submit-note` — merged into the submit button's ring; a separate label would sit over adjacent Description content). **The narrative MUST name all three in prose** — the section rail in the editor overview section, the "Section N of M" position indicator in the *Saving your work* section, and the submit note in the AI-quality-check section. Without those sentences the exception is unsatisfied and `BG-R-6` is unmet.
  2. ⚠️ **Do NOT assert a specific field value as guaranteed.** `BG-T-9` observed a **reproducible client-side timing race**: across three otherwise-identical read-only pipeline runs on the same result, *Contributors & partners* (external-partner chips and their validation) and *Geographic location* (radio selection) rendered **differently** — sometimes complete, sometimes showing a transient "field(s) missing" state. Heading text was stable every time, so the caption gate never saw it. Therefore the narrative must not say things like *"you will see WUR and MARI listed"* or *"this section shows as complete"*: write what the **control** does, never what a particular capture happens to contain. This is defect class **D9** — a plausible-but-false sentence, with no automated gate.
...
### `BG-T-12` — Content: sections 16–18, statuses, and the glossary  `[x]`
...
- **Verification:**
  ...
  - 🛑 **CARRIED FROM `BG-T-10` (2026-09-21) — a structural gap NO task owned, and `build-guide` cannot succeed until this task closes it.** `assemble.ts`'s `SECTIONS: SectionMeta[]` is still the **verbatim W1/W2 metadata** (`01-landing.md` … `06-innovation-packages.md`), so assembly reads filenames that do not exist here. Worse, its model pairs **exactly one capture per section** (`routeCaptionKey: string`), which was true of W1/W2 (6 sections, 6 captures) and is **false here**: this guide has **18 sections and 17 captures**, and the mapping fans out —
    ...
    **Two stale tables, not one — both are this task's.** Besides `assemble.ts`'s `SECTIONS`, **`verify-structure.ts:39-48` also still asserts the W1/W2 section ids with `EXPECTED_TOTAL_H2 = 9`** (found by `BG-T-12`'s Reviewer; the Leader had assigned only the first). Fix **both**, or the build fails — or worse, the verifier passes green over the wrong section set. **This task owns the fix.** Rewrite `SECTIONS` for this guide and widen `SectionMeta` so a section carries **zero or more** capture keys (e.g. `routeCaptionKeys: string[]`) instead of exactly one. The array's comment calls the order *LOAD-BEARING* against `template/README.md`'s heading-count contract — so re-read that contract and keep `verify-structure` honest rather than relaxing it to fit. **Falsifier:** remove one section's entry and confirm `verify-structure` reddens on the missing heading; restore and confirm green. Do **not** reshape the guide's sections to fit the tool — `proposal.md` §4's structure is the approved deliverable and the tool is what adapts.
