...
| `BRV-AC-2` | 1536, ≥ 80 rows, work area scrolled by 600 (`scrollTop === 600` asserted) | Measure | `pinned.top === workArea.top ± 1`; `filterBand.bottom === pinned.bottom ± 1`; `statbar.top < pinned.bottom`; `window.scrollY === 0`; pinned height ≤ 150 (measured 142); FAIL input: drop the `sticky` class → `pinned.top < workArea.top` (RED recorded). (Band hero top unchanged is documentation — it is a sibling above the work area and cannot move.) |
...
| `BRV-AC-13` | 1787 live (owner's screen) | Before/after | Visible data rows within the first screen ≥ the "after-polish-1" count; pinned chrome ≤ 150 px |
