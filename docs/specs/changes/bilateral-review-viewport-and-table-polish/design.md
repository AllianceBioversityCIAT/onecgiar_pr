...
- `.html`: keep the root `section`/`article`; wrap toolbar + filter band in `<div class="min-[900px]:sticky min-[900px]:top-0 min-[900px]:z-[15] bg-[var(--pr-surface-app)] border-b border-[var(--pr-border-divider)]" data-testid="bilateral-review-pinned">`; the filter band's own `border-b` moves to the wrapper; the toolbar keeps `bg-[var(--pr-surface-card)]`. No `overflow` on the wrapper. `z-[15]`: above the rows' `z-10` sticky Actions cell, below the band's `z-20`/`z-[22]`/`z-[30]`. Rows area gets `scroll-margin-top: var(--brv-pinned-h, 142px)` on `tr`/cards (a CSS var the page sets from the wrapper's measured height via a `ResizeObserver`, or a fixed 142 px fallback). Remove the false "same viewport-lock contract" comment at `.html:18-19`.
