...
| Design tokens | Only `var(--pr-*)` tokens from `docs/ux-ui/design.md` §7 in new markup (the pre-existing raw amber/emerald/red status pill is untouched). Icons: `docs/ux-ui/design.md` §7 line 230 says `material-icons-round` and never mix sets; the bilateral page **already mixes both sets** (lucide for search/expand/clear affordances and the band; `material-icons-round` for four pre-existing toolbar glyphs, the table's row action and group chevron) — **execution correction, BRP-T-4:** the premise "the toolbar is all lucide" was false. The rule this spec adopts: **no new markup mixes sets within one control; each new control follows the set of its nearest existing siblings** (band/strip = lucide, table/cards = `material-icons-round`, stat bar = no icons); the pre-existing toolbar glyphs are grandfathered — recorded as a deviation in `onecgiar-pr-client/docs/DESIGN-DEVIATIONS.md` (T-4) |
...
| `BRP-AC-7` | 1536 px (CT) and the real page (HITL) | Page loads | CT: `[data-testid="bilateral-review-filter-band"]` height + stat bar height ≤ **140 px** with the centers row collapsed, and `firstRow.top − workArea.top` ≤ **231 px** (**execution correction, BRP-T-4:** measured 223 px to the first group-header row in CT and 221 px live — the 210 estimate omitted the table header; gate = measured + 8); FAIL input: inject `min-height: 300px` on the band → both fail. HITL: first row top < **420 px** from the viewport top at 1130 px tall (today 494) |
...
| `BRP-AC-11` | 840 px (CT `cy.viewport(840, 1600)`; the 9-card fixture at `(840, 2400)`), grouped | Renders | Rows are cards; no `<table>`; body `scrollWidth <= clientWidth`; `firstCard.top − workArea.top` ≤ **282 px** (**execution correction, BRP-T-4:** measured 274 px — the toolbar wraps to two rows at 840 once the Group control exists; gate = measured + 8); no descendant of `#workArea` with computed `overflow-y: auto\|scroll` |
