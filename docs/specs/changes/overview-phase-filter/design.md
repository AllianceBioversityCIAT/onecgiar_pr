...
- **Selector** (OPF-R-1): PrimeNG select in the Overview header band (next to the cycle eyebrow that already reads `reportingCurrentPhase`), tokens per `docs/ux-ui/design.md` §7; options = `sp.versions` sorted `phaseYear` desc; option label "«phaseName» · «phaseYear»"; an "Open" tag on the option whose `versionId === reportingCurrentPhase.phaseId`. Keyboard/reader semantics come free with the component (OPF-N-2).
