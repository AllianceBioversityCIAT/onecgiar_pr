...
### Hotfix h1 — HITL finding: no skeleton on phase switch (2026-08-28)
- Owner visual check: everything approved EXCEPT "cuando cambie de fase y debe cargar debería aparecer el skeleton o loader" — OPF-R-2 loading clause regression, invisible to jsdom (defect class D4 doing its job).
- Leader diagnosis (code-level; Chrome extension unavailable for live repro): loading computeds depend on in-flight key-sets populated inside effects that run AFTER the render triggered by the selection write → a window where data reads the new (empty) key while loading is still false → `@if (data || loading)` gates fall to empty state.
- Fix direction dispatched to the T-4 Implementer: cache-presence-based loading (`!cacheByKey().has(key)` while a fetch is warranted) — removes the timing hole by construction; error paths still cache a value so OPF-R-5's no-infinite-loader holds. Red→green immediacy test (h) required per computed + error-path test (i).
- **Implementer delta:** root cause confirmed empirically (red at spec.ts:1454 — loading computeds false in the pre-effect-flush window); 5 computeds inverted to cache-presence (`!cacheByKey().has(key)` + fetch-warranted gates); in-flight sets KEPT as HTTP de-dup guards; tests (h) immediacy red→green + (i) error settles. Scoped Jest 418/418; lint + tsc clean.
- **Reviewer verdict: PASS** — every cache-miss maps to a fetch that settles the same key on next AND error (all 4 loaders cited); key identity computed↔loader verified (`versionId ?? 'default'` normalization); no loader without a fetch; default path request-pattern byte-identical (the new default-path skeleton on the W1/W2 heatmap replaces the empty-state flash — OPF-R-2 requires it, OPF-R-3 constrains values/requests, not transient affordances).
- ADVISORY (polish follow-ups, recorded): `overviewTocMapLoading` lacks the view gate its siblings got (harmless today — sole consumer renders under Overview only); parameterize test (i) across the other 3 loaders to enforce the no-stuck-loader invariant.
- Landed: commit below.

### Hotfix h2 — HITL finding: cards stuck on the Open phase under a 2025 selection (2026-08-28)
- Owner cotejo: 2026 perfect; 2025 → W1/W2 tile/matrix/ToC stuck at 2026 values while bilaterals showed 2025 data (81). Mixed behavior = the diagnostic key.
- Forensics en route (recorded): served-bundle audit chunk-by-chunk ruled out staleness (dev server restarted after the trend session's type break — LineChart registered additively, commit 3be2149f8, unblocking their in-flight feature); ResultToReview probe (v34=11 groups / v36=6 / none=15) ruled out the bilateral card as stuck.
- **Root cause (probe-confirmed):** `GET /api/versioning` serializes `version.id` as a **STRING** (bigint column). `phaseSelectorOptions` mapped `versionId: p.id` raw → the selection propagated as `"34"` → every OPF wrapper's strict `typeof === 'number'` guard silently dropped the param (fetch without versionId ⇒ Open-phase data cached under the 2025 key), while the legacy lenient guard in `GET_ResultToReview` passed it — exactly the observed split.
- **Fix (Leader-inline, owner-present fast mode — recorded exception):** normalize at the single origin (`versionId: Number(p.id)` in `phaseSelectorOptions`) rather than loosening four guards; spec fixture flipped to the REAL wire shape (string ids) — red-proof run: reverting the normalization fails 2 tests (421/423), restored ⇒ 423/423. Lint clean.
- Lesson candidate (kaizen): KZ-TCM-1 extends to the TYPE axis — wire-shape fixtures (string bigints) are mandatory when a guard discriminates on typeof.
