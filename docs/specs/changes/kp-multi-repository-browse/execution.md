...
| Field | Value |
|---|---|
| Status | HITL executed on the **local stack**; Reviewer audit of the evidence: **FAIL on two documentation points, both closed below by the Leader**; the QA-environment run stays owed → task stays `[~]` |
...
| 6 | *Use this item* (MEL) → handle / MQAP / banner | **NOT TICKED — banner unverified live** (MQAP / `itemUrl` / metadata sync confirmed) | MQAP called with `https://repo.mel.cgiar.org/items/53d67b9b-…` (regex-valid, `KPM-AC-12`); title/authors/type synced from MELSpace; the AoW drawer **auto-created draft Result #9146** (EDITING, unsubmitted) and navigated to the editor, so the "Selected from MELSpace" banner was not observed live (host unit tests assert it). Draft left in the local dev DB; "Submit" never clicked |
...
**Reviewer audit of the HITL evidence (`opus`) — FAIL (documentation), closed by the Leader**

- Evidence integrity judged strong: every status claim backed by its `sources[]` JSON; screenshots match the tables; Run A provably uncontaminated by the blackhole (all three `ok`); Run A / Run B kept apart — disqualifier satisfied. Judgments: the local run discharges the §9 "real-API drift / visual fidelity / keyboard" rows on the merits (real upstreams, same client build); **still owed and not substitutable:** the QA-environment run proving `serverless.yaml` delivers the two env vars into the Lambda (`KPM-R-13`, `KPM-AC-9` in the deployed shape). Item 4 NOT OBSERVED acceptable (three strategies tried; the uncollapsed same-title/different-type pair is positive `KPM-R-5` evidence; mechanism covered by `merge.spec.ts` + T-7). Item 6 is **not a PRODUCT_BUG** (pre-existing AoW auto-create; `itemUrl`, MQAP and sync correct) but the banner is a host-template clause, so "same component" was not a sound reason to skip the `lab-report-form` host — `KPM-AC-12`'s banner has no live confirmation in any host. `unconfigured` not exercised live — acceptable (T-4 (d)/(e′)); folds into the QA run.
- **Issue 1 — Discovered Issue:** drift recorded only as narrative, never filed. **Violated Rule:** `KPM-T-10` DoD "any drift filed as a follow-up in `design.md` §13". **Remediation applied:** `design.md` §13 now carries execution follow-ups (a)–(h), including (f) AoW auto-create-before-banner and (g) idle-strip layout vs mockup (spec's own file — exempt from the shared-file discipline).
- **Issue 2 — Discovered Issue:** `KPM-AC-12` banner clause has no live confirmation. **Violated Rule:** `KPM-T-10` Implements "live confirmation of … `KPM-AC-12`"; `requirements.md` AC-12. **Remediation applied:** item 6 restated as not ticked (MQAP / `itemUrl` / sync confirmed, banner unverified live); the QA run must carry one *Use this item* on a MEL item through the `lab-report-form` host and capture the banner text.
