...
### [x] `BIL-QAI-T-1b` — Contract v0.2 copy and the per-type label enumeration for Daniela
...
### [x] `BIL-QAI-T-4b` — Type-specific mapper v0.2, label constants, impact-areas mapper, tag-vocabulary fix
...
### [x] `BIL-QAI-T-6` — Orchestrator, shared `assertSubmittable`, running lock, `POST` and `GET latest` endpoints
...
### [x] `BIL-QAI-T-12` — Innovation developers field restored in the bilateral form
