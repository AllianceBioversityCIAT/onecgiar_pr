...
| **P-5** | The detail payload already carries `creation_method` and `is_ai_generated`; only the **client interface** fails to declare them | `existence` | `result.repository.ts:3406-3407` selects both; `BilateralCommonFields` at `result-review-drawer.interfaces.ts:52-65` declares neither | `da132347c` | **Low** — if the detail SQL lacked them, `BSR-T-5` grows a server edit | — |
...
| `bilateral-review-table.component.spec.ts:562-570` | Alignment `td` keeps class `min-w-[220px]` | → `min-w-[192px]` (DD-2's new Alignment width) |
...
['96px', '']                               // code, title (unset — absorbs the remainder)
  ...
  + push '120px', '192px', '100px', '100px'  // status, alignment 220 → 192, date, actions
```
...
**Widths.** Project mode fixed sum `746 → 812` (`+66`); center-grouped `636 → 724`. Title loses 66px, not 116.
**Why.** Title must stay the widest column at 1000px and 1280px (BRV-R-3, P-7). Projected: ~464px @1280, ~184px @1000 — still the widest at both. Lead Center holds a centre acronym (`CIMMYT`, 6 chars) in a 110px box; Alignment at 192px keeps its two truncated lines.
...
**⚠️ The existing cap gate is VACUOUS for this DD until its fixture changes (Reviewer finding, round 1).** `bilateral-review.cy.ts:1144-1177`'s three fixture rows carry neither `reporter_name` nor `creation_method`, and the `row()` helper (`cy.ts:95-109`) defaults neither — so "the three caps pass unchanged" today would measure a **single-line** date cell and a **placeholder** SOURCE cell, proving nothing about either new surface. `BSR-T-6` therefore requires the fixture to gain a `reporter_name` on all three rows **and** one row with `creation_method: 'AI'` (the tallest SOURCE content), with the caps then holding at 46 / 50 / 68. Without that fixture change the gate is an inert fixture in the exact sense the task rules name.
