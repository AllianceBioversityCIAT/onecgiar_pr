...
| `BilateralOverviewService` | `providedIn: 'root'` | `Map<'center::version', {results, projects, loadedAt}>`; `load()` returns cached or fetches; exposes per-key `loading`/`error` signals; `invalidate(center)` on retry. Drafts are **not** fetched here — read from `BilateralAiService`. |
