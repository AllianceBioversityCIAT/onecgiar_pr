...
### L3 — Time comparisons across hosts must happen in DB time (post-archive defect, 2026-09-16)
- **Root Cause:** `created_date` / `queue_entry_date` are generated by MySQL (UTC on prtest), but every timestamp the app wrote (`started_date`, `stage_updated_date`, `completed_date`, `retried_date`) and every sweeper cutoff (`new Date(Date.now() − ms)`) was serialized by mysql2 in the Node process's local zone (no `timezone` in `orm.config.ts`). The write-side skew predates the spec (rows from 2026-09-14 already carry `started_date` 5 h behind `created_date`); the new sweeper turned it into false outcomes as soon as the consumer (developer laptop, America/Bogota) and the sweeper (prtest Lambda, UTC) shared one DB: job `6716bf59…` was flipped to `TIMED_OUT` 48 s after it started, and the stall liveness guard never saw a "recent" `stage_updated_date`, so `QUEUE_STALLED` could fire while a consumer was alive. Unit tests with repository mocks and frozen clocks cannot see this — the skew lives in the driver's serialization, not in the code under test — and the T-10 HITL passed because consumer and sweeper both ran on the same laptop.
- **Evidence:** read-only query of `bilateral_ai_jobs` on 2026-09-16 (`c6b13dc8…` created `12:43:20` UTC, `started_date` `07:43:21`); `docs/specs/archive/2026-09-15-bilateral--ai-processing-feedback/requirements.md` `APF-OQ-7`; fix commit `[SPEC:bilateral/ai-processing-feedback]` "lifecycle timestamps and sweeper cutoffs in DB time".
- **Classification:** Product (latent driver/time-zone inconsistency) + Methodology (HITL ran consumer and sweeper on one host, so the multi-host case was never exercised).
- **Action:** Any server logic that compares a DB-generated timestamp with an app-written one must write with `CURRENT_TIMESTAMP` and compare in SQL (`DATE_SUB(NOW(), INTERVAL … SECOND)`, `TIMESTAMPDIFF`), never with a JS `Date` — unless `orm.config.ts` sets `timezone: 'Z'` project-wide (a separate decision with its own regression sweep). Specs whose runtime spans two processes (consumer + cron, API + worker) must include a HITL row where the processes run on hosts with different `TZ`, or state that they were run on one host and why that is acceptable. Local development of this module: run the API with `TZ=UTC` until the ORM decision lands.
