...
|---|---|---|
| `click` | `selector`, optional `label` | selector must match **exactly one** element, else the run fails. Since Pivot 3 a step's selector may be `[data-guide]`/`[data-testid]` **or** `role=<role>[name=…]`; never raw text, `:nth-of-type`, `.first()` or position |
...
| `BG-DD-4` | Anchor **callouts** on `[data-guide]`/`[data-testid]` only; **interaction steps** may additionally use an ARIA `role` + accessible name (see the Amendment below) | 11 + ~20 such anchors already exist, were added deliberately with zero layout impact, and are unit-guarded against removal (§11 `P-2`, `P-3`). The W1/W2 config's `text=`/`:nth-of-type()` selectors are why its routes broke | **Text selectors** — break on copy edits, and `BG-R-6` demands a unique match. **XPath/position** — worse on both counts |
...
**Amendment to `BG-DD-4` (Pivot 3, `BG-T-8`, 2026-09-21 — operator-approved).** The drawer's own choice controls carry **no per-element `[data-guide]`/`[data-testid]`**: the Science-Program rows are an `@for` over `availableSps()` with only a shared class, the creation-method cards carry `role="button"` and no per-card id, and the result-level selector has no anchor at all. They are 2+-way choices among structurally identical siblings, so `BG-T-4`'s uniqueness guard **correctly** rejects a class selector (count = 2) — the gap is in `P-2`/`P-3`, which never enumerated drawer-internal anchors, not in the implementation.

Resolution: **interaction steps** (`click`, `fill`, `waitFor`, `press`) may resolve a target by **ARIA `role` + accessible name** — e.g. `role=radio[name=/SP01/i]`. **Callout anchors are unchanged** and remain `[data-guide]`/`[data-testid]` only.

Why this is not a relapse to text selectors: `docs/ux-ui/design.md` §10 obliges the app to **WCAG 2.1 AA**, so `role` and the accessible name are a maintained contract rather than incidental markup — a different stability argument from raw text. Still forbidden, unchanged: raw text selectors, `:nth-of-type`, `.first()`, `.nth()`, XPath, and any positional resolution.

**Costs zero lines of code.** Verified by probe: Playwright's `role=` engine resolves through the same `page.locator()` that `assertUniqueSelector` already calls, and the guard keeps rejecting the ambiguous forms — `.sps-option` → 2, bare `role=radio` → 2, `role=radio[name=/SP01/i]` → 1, `role=radio[name=/Genebank/i]` → 1.
...
**TRIPWIRE FIRED 2026-09-21 at `BG-T-9` close — review rounds 19 vs 17 budgeted (LOC ~1,405, within range). Escalated to the operator before crossing 20; operator chose to continue in full and the three cuts were declined. See `execution.md` → *Budget tripwire*.**
