...
### `SIP-T-7` — Visual styling correction for the merge/split picker

- **Final status:** PASS
- **Date:** 2026-09-16
- **Implements:** client-side visual correction, traces to `SIP-US-2` and `proposal.md`'s original "carry over the visual language" scope (see Pivot Record above)
- **Implementer attempts:** 2

#### Attempt 1

- **Skill:** `spartan` + `angular-developer` · **Effort:** high
- **Files changed:** `pr-multi-select.component.ts` (new `resultPickerStyle` boolean input, off by default), `.html` (`[class.result_picker_style]` binding on `.options`), `.scss` (new scoped block: rounded panel + shadow, bordered search bar with inset icon, option-row padding/divider/hover), `rd-annual-updating.component.html` (`[resultPickerStyle]="true"` on both merge/split instances), `pr-multi-select/CLAUDE.md`.
- **Incident (see Pivot Record's incident note above, not repeated here):** the Implementer's diff also silently rewrote `optionsIntance()` — caught and reverted by the Leader directly before the Reviewer spawn, with 103/103 Jest + `ng lint` re-verified clean after the revert.
- **Reviewer verdict:** **FAIL** (one issue, unrelated to the incident, which the Reviewer separately re-confirmed was cleanly reverted). The option-row `padding: 16px 20px` was visually inert: a global `.custom_select .field .options .virtual_scroll .option { height: 30px; ... }` rule (`custom-fields.scss`) combined with the CDK virtual-scroll viewport's hard-coded `itemSize="30"` pinned every row to 30px regardless of the new padding — under border-box sizing, 16+16px padding on a fixed 30px box leaves a 0px content box, so the "spacious divided rows" effect never rendered. Panel/search-bar styling and the `optionsIntance()` revert were both confirmed correct in the same pass.

#### Attempt 2 (rework)

- **Skill:** `spartan` + `angular-developer` · **Effort:** high
- **Feedback passed verbatim:** Reviewer's Issue 1 (inert padding, root-caused to the fixed row height + `itemSize`), with the remediation pointing at `pr-select.component.ts`'s existing `virtualOptionItemSize` computed-value pattern as precedent.
- **Files changed:** `.scss` — added `height: auto; min-height: 52px;` inside the already-approved `.option` block (16px+16px padding + 18px `pr-body-2` line-height + 1px border-bottom = 51px, +1px buffer). `.ts` — added `readonly virtualOptionItemSize = computed(() => (this.resultPickerStyle() ? 52 : 30));`, mirroring `pr-select`'s exact pattern (name, shape, doc-comment, 30px fallback). `.html` — `cdk-virtual-scroll-viewport itemSize="30"` → `[itemSize]="virtualOptionItemSize()"`. `CLAUDE.md` — corrected to state the actual rendered min-height (not just padding) and the "must stay numerically in sync" pairing requirement.
- **Verification:** `ng lint` clean; 103/103 Jest tests unchanged (layout-only fix, structurally invisible to jsdom); `optionsIntance()` re-confirmed byte-for-byte the original shape (no trace of the reverted rewrite); diff confirmed scoped to exactly the described fix.
- **Reviewer verdict:** **PASS** (two independent review passes reached the same conclusion — an earlier attempt at this same review hit a rate-limit failure mid-run with no verdict, and was re-run fresh; a stray duplicate completion notification from the failed attempt's underlying process arrived after the fact and was disregarded since it carried no new information). Re-derived the CSS math independently: confirmed `min-height` is not declared anywhere in the global rule at all (only `height`), so per CSS the row's used height is clamped by the component's `min-height` regardless of which `height` declaration wins the specificity tie between the global and scoped rules — the fix does not even depend on the specificity reasoning attempt 1's review used, making it more robust than originally argued. Confirmed the `[itemSize]` binding is legitimate CDK usage (one fixed size per viewport instance, not per-row — exactly how `pr-select` already uses it), confirmed it lives only in the flat-mode template branch reading the same `resultPickerStyle()` signal as the panel class, and confirmed genuine fidelity to the `pr-select` precedent.
  - **RECORDED GAP (both review passes flagged this identically, non-blocking but explicit):** nothing automated measures the actual rendered row height — `pr-multi-select.cy.ts` has no case for `resultPickerStyle`, and Jest/jsdom cannot evaluate layout. The Reviewer's PASS rests on independently re-derived CSS math (specifically: `min-height` is uncontested by the global rule), not on a runnable measurement. The task's own DoD closes this with "manual browser confirmation (by the user)" — which is the appropriate substitute per this spec's established pattern (per `design.md` §10's "Visual match... no automated check... substituted: manual verification in a real browser").

**Requirements covered:** restores the visual-language intent from the original `proposal.md` (search bar + scrollable list styling) that `design.md`'s `RES-DD-2` had over-narrowed, without adding the explicitly-out-of-scope filter-chip set.

**Decisions made:** kept the variant strictly opt-in (new boolean input, default `false`) rather than a global restyle, preserving the "other ~78 instances unaffected" invariant established since `SIP-T-3`; used a `min-height` override + paired `virtualOptionItemSize` computed rather than an inline style (the alternative `pr-select` itself uses for its own analogous case), judged the cleaner of the two by the Reviewer.

**Issues encountered:** (1) the `optionsIntance()` incident, caught and reverted directly by the Leader, not the Reviewer — recorded as a process finding, product feedback filed; (2) a real, twice-independently-confirmed layout defect (inert padding under a fixed row height) that required a second rework attempt.

**Final verification result:** PASS on the automated/code side (lint clean, 103/103 tests, CSS math independently re-derived twice). The task's manual-browser DoD item was outstanding under this session's own tracking at the time an external session archived, committed, and pushed the spec (commit `b18c365c3`, merged via `6a1cb5937`/`2e1eb9420`) — confirmed by the user to be their own action in a separate session. **Note for the record:** the archive's own `archive-summary.md` states "Completed (PASS)" for `SIP-T-7`; this entry is the fuller account of that task's actual attempt history, including the incident and the recorded verification gap, for anyone auditing this spec later.

#### Follow-up: closing the row-height verification gap (post-archive)

- **Date:** 2026-09-16 · **Skill:** `angular-developer` · **Effort:** medium · **Attempts:** 1 (PASS)
- **Files changed:** `pr-multi-select.cy.ts` — two new Cypress CT cases (additions only): (1) mounts with `[resultPickerStyle]="true"`, opens the dropdown, measures a real rendered `.option` row's `getBoundingClientRect().height` and asserts it lands in `[51, 54]` (tolerant of box-model rounding around the shipped `min-height: 52px`), plus asserts `.options` carries the `result_picker_style` class; (2) mounts the default/unwired instance and asserts BOTH the class is absent AND the row height stays `~30px` (the pre-existing global value) — proving the opt-in default holds.
- **Verification:** `npx cypress run --component --spec ".../pr-multi-select.cy.ts"` → 11 tests, 10 passing (both new cases green), 1 known/pre-existing failure (the already-triaged `NG0100` splice-regression test, re-confirmed unrelated via `git stash` against the unmodified file — same finding as `SIP-T-4`'s investigation). Diff additions-only. Lint clean.
- **Reviewer verdict:** **PASS.** Independently confirmed the CT dev-server loads the same global `custom-fields.scss` cascade production uses (`cypress.config.js:152`, mirroring `angular.json:43`), so the measurement is genuinely load-bearing — a reverted fix would land the row back at 30px, well outside the asserted band, and the unwired case would double-fail (class + height) if the input's default ever flipped. No transition/animation risk on the `.options` reveal (`NoopAnimationsModule` + no CSS `transition` on that element) that could make the non-retrying measurement flaky.
  - **Remaining non-blocking gap (explicitly named, not this follow-up's scope):** the panel's rounded corners/shadow, the row divider/hover, the search-icon position, and — most worth a future case — the `virtualOptionItemSize() === 52` ↔ SCSS `min-height: 52px` pairing itself (this follow-up tests the SCSS side's rendered effect, not that the `[itemSize]` binding numerically matches it) remain unverified by any automated check. `SIP-T-7`'s own `CLAUDE.md` note ("if either value changes, change both") is the only guard against drift today.

**Manual browser DoD item status:** the automated row-height gap is now closed by the above. The task's own DoD line ("manual browser confirmation by the user that the picker now visually resembles the reference") is a separate, still-user-owned checklist item for the panel's overall visual feel (rounded corners, shadow, icon placement, spacing) that no automated check substitutes for — per this spec's own established pattern (`design.md` §10). Left as the one open item for the user's own final look.
