...
### Addendum (user request, 2026-08-29) — AoW context banner on the "By AOW" reporting view

- Owner: landing on the Reporting tab after a hub "Report" click gives no cue of which AoW you are working in (competitor reference: an AoW header card with KPI stats). Added a context banner at the top of the `byAow` branch in `dashboard-lab.component.html`: AoW code chip + full name (`prTooltip`), an "All Areas of Work" back button (`setPlannedBrowseView('aows')`), and three stat tiles — KPIs, Reported x/y, Progress % with bar — computed by the new `plannedAowBanner` computed over the AoW's output-tier indicators (same rule as `overviewAowProgress`); tiles skeleton while `plannedHloLoading()`. Pure helper `buildAowBannerStats` exported and unit-tested (+2 tests, incl. empty-list → 0% not NaN). `PrTooltipDirectiveModule` added to dashboard-lab imports. Leader-inline, ~90 LOC.
- Deliberately NOT copied from the competitor: "Off Track" status and "Generate Narrative" (no such data/feature in PRMS today — candidates for a future proposal).
- Verification: 47/47 dashboard-lab targeted tests green; `npx ng lint --quiet` clean; `npx ng build --configuration development` OK.

### Addendum (user request, 2026-08-29) — By-AOW view: outcomes tier + working Section filter

- Owner reported (a) the Section filter does nothing on the By-AOW view and (b) the view lists only the output tier (HLOs) while the grouped table shows Outputs + Outcomes — the context banner also under-counted (53 vs 69 KPIs).
- Fixes: new `plannedByAowSections` computed renders the view as tier sections ("High Level Outputs · N KPIs", "Outcomes · N KPIs") reusing `groupIndicatorsByHlo`/`rankPlannedHloGroups` per tier (pure `splitIndicatorsByTier` exported + tested); the banner now counts all tiers; `onSectionFilterChange` keeps the grouped-view semantics and, on By-AOW, acts as the AoW switcher (first selected AoW code → `setPlannedHloAow`; only programme-level buckets selected → back to the grouped view). Empty-state copy updated ("No indicators match…").
- Gap recorded: `onSectionFilterChange`'s By-AOW branch has no unit test (exercising it needs the full toc-load machinery); covered by the T-7 manual checklist (new item: pick a Section on By-AOW → view switches to that AoW).
- Verification: 48/48 dashboard-lab targeted tests green; `npx ng build --configuration development` 0 errors; `npx ng lint --quiet` clean. Leader-inline, ~80 LOC.

### Addendum (user request, 2026-08-29) — By-AOW indicator cards redesigned
