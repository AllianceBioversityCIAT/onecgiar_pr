...
### `MWB-T-11` — Responsive board below the viewport-lock breakpoint — **PASS** (attempt 1, resumed after a session restart) · 2026-09-05

- **Implementer:** `akili-implementer` on `opus` · skills `angular-developer`, `frontend-design`, `tdd` · effort high. Files: `my-work-board.component.{ts,html,scss,spec.ts}`, `my-work-board.cy.ts`, `components/my-work-column/*`. Mechanism: CSS `max-[899px]:`/`min-[900px]:` for visuals + `isNarrow` signal (`matchMedia`, `DestroyRef` teardown) for structure (Closed columns render as sections, no rails/jumper/collapse controls below 900). Floors: Editing 320 / `min-w-[240px]` below 1440, 360/260 at ≥ 1440 (`MWB-T-10` pointer). Trap fixed: unconditional `flex-1 min-h-0` collapsed the board to 0 px below 900 → gated to `min-[900px]:`. Two browser-found defects fixed and gated: `min-[900px]:shrink` beat `flex-none` (Editing 356 px) → removed + CT 1000×700 asserts Editing == 320; filter popover off-screen at 390 (doc `scrollWidth` 598) → `max-[899px]:w-full` on the filter container + narrow CT cases open the popover and assert no doc overflow. Hit targets ≥ 44 px applied from the parent SCSS (`:host ::ng-deep app-my-work-card button/a` under `max-width: 899px`) because the card file is owned by another session — **temporary**; card-side classes for the owner: `max-[899px]:min-h-[44px]` on the two action buttons and `max-[899px]:inline-flex max-[899px]:min-h-[44px] max-[899px]:items-center max-[899px]:px-[8px]` on the Open anchor, then delete the block.
- **Verification:** Jest `7 / 127`; tsc; lint; CT `6 passing` (1280, 1440, 1280-expanded, 1000 overflow, 390×844, 768×1024). Browser (Orca, proxy now on :53131): effective 390 → `documentElement.scrollWidth 372 ≤ 390`, strip 1753/372, 5 columns at 331.5 px (85vw), no rails, 5 jumper chips @ 44 px, Submitted chip → delta 0, Continue/Open 44 px; 768 → columns 360, jumper sticky flush under the band (253 px); 1280 with sidebar → 320 / 240×3 / rail 44 (strip 1212 vs client 1020, document 1280); 1440 → 360 / 260×3 (1312 vs 1180). **Floor arithmetic:** fitting the collapsed default inside 1020 px would need 176 px columns → below the 240 readable floor → not done; the board keeps its own horizontal scroll (`MWB-R-9` holds: document never overflows).
- **Reviewer (`akili-reviewer`, opus):** `STATUS: PASS` — "All six `MWB-T-11` description items are implemented as specified and are backed by real-layout evidence … `MWB-R-9`'s negative clause holds at every audited viewport, the `isNarrow` listener is torn down through `DestroyRef` … `MWB-DD-8` / `MWB-T-10` behaviour is byte-identical at ≥ 900."
- **ADVISORY (recorded → `pending-archive.md` follow-ups):** fractional widths in `899 < w < 900` fall between the CSS and JS predicates (use `899.98px` on both sides); cancel the pending `requestAnimationFrame` on destroy and skip `onStripScroll` when not narrow; the jumper's sticky `top` hard-codes the band's 64+48 geometry (band should publish its height as a CSS var); `::ng-deep` hit-target block is a stand-in until the card owner applies the classes; two cheap Jest cases (teardown, reduced-motion branch); docs synced by the Leader (design §6.3 row, tasks T-11 item 5, spec header markers).
- **Gate:** auto-approved (pre-approved mode).

### Quick-track: card header wraps instead of overflowing the column (user: "revisa el responsive", screenshot 2026-09-05) · Leader inline

- **Cause:** the earlier chip fix (`shrink-0 whitespace-nowrap`) made the code · category · status · origin row incompressible, so at the 240/260 px floors cards spilled past the column and every list grew a horizontal scrollbar (texts clipped on the right).
- **Fix:** header row `flex min-w-0 flex-wrap items-center gap-x-[8px] gap-y-[4px]`; category chip `inline-block h-[16px] max-w-full truncate leading-[16px]` (ellipsis when it alone exceeds the row); status chip keeps `whitespace-nowrap`, loses `shrink-0`; origin `ml-auto whitespace-nowrap`; column list `min-[900px]:overflow-x-hidden` (gated so the < 900 `overflowY: visible` CT assertion holds).
- **Verification:** Jest `7 / 127`, lint clean, CT `6 passing`. Live (SP01, All, 147 rows): effective 2000 → lists `scrollWidth == clientWidth` in all four columns, 0 clipped chips; effective 1280 → same, 4/3 category chips ellipsized in the 232 px lists (intended), rows wrap where needed, document width == innerWidth.
- Committed via blob staging again (foreign `rememberOrigin` hunk kept out and intact in the working tree).

### `MWB-T-12` — Category, Funding source and Contributing Center as multi-selects — **PASS** (attempt 1) · 2026-09-05

- **Implementer:** `akili-implementer` on `opus` · skills `angular-developer`, `onecgiar-pr-client:spartan`, `tdd`. Files: `services/my-work-board.service.ts` (three `string[]` signals next to `visibleRows()`; OR within / AND across after `filter.filterRows`; `matchesProgrammeResultCategory` for `__other__`, `normalize()` equality for origin/center), `my-work-board.component.{ts,html,spec.ts}` (`app-pr-filter-multiselect` ×3 in the popover, `boardChips()` one chip per value, `clearMultiFilters`, URL `category|origin|center=a,b` with split/trim/dedupe and order-sensitive `sameList`), `my-work-board.cy.ts` (fake service extended, no selector change). Shared service's single-select signals stay `null`; `programme-results/**` untouched.
- **Verification:** Jest `7 / 139`; tsc; lint; CT `6 passing`. FAIL-input probe `.some → .every` made 5 page cases fail (fixture 3 KP / 2 Innovation development). Browser (SP01, All, 1280): KP + Policy change → 35 cards = 26 + 9 (OR), badge 3, URL `category=Knowledge product,Policy change`; both origins → 9 cards (AND across); chip × drops one value; Clear filters → 145 cards; multiselect inherits the `.mwb-filter` reshape (34 px field, panel +4 px).
- **Reviewer (`akili-reviewer`, opus):** `STATUS: PASS` — "OR within Category / Funding source / Center and AND across them … one chip per value with per-value removal, comma-separated URL bridge that hydrates and mirrors without oscillating, `Other` semantics and option lists reused from the shared helpers, and the shared `ProgrammeResultsFilterService` single-select signals left at `null` and unmodified."
- **ADVISORY (recorded → `pending-archive.md`):** a comma inside a server-supplied centre/funding label breaks the URL round-trip (encode per element or change the separator when `MWB-T-13` lifts this into the shared service); `withSelectedOptions` dedupes by exact string vs `normalize()` filtering; `sameList` doc comment says "order-insensitive" but is order-sensitive (intended).
- **Traceability note:** the board page already imported `WhereToReportModalComponent` at HEAD (the in-place Where-to-report modal); that hunk came from the concurrent session's edit and was swept into an earlier board commit of this spec — recorded here so the archive does not attribute it to a T-N task. T-12 only stubbed it in the spec harness.
- **Gate:** auto-approved (pre-approved mode).

### Take-over of the concurrent session's uncommitted work (user: "toma tú los archivos y sigue con T-13") · 2026-09-05

- Coordination messages to the other sessions expired unapproved; the user authorised the take-over. Verified first: `tsc` clean, lint clean, targeted Jest on every touched suite → `1 failed / 410 passed`. The failure was internal to that work: new `DashboardLabComponent.onResultCreated()` navigated back to `returnTab` while its own spec asserted it must not (the report modal owns the post-creation navigation) → fixed the implementation to match the spec (`2 suites / 120 tests` green).
- Committed in four intent-sized commits, each marked "taken over from a concurrent session": `0e400a68d` sp-shell (guided-tour step + tab badge style + `returnTab=my-work` on Where-to-report / report-emerging), `3c28f307c` smart-navigation (result-detail Back → My results when opened from a card), `cc0aadf66` pr-filter-select (in-panel search for long lists), `5a37308a3` programme-results (in-place Where-to-report modal, row hover surface).
- Consequence for the spec: the earlier traceability note (`WhereToReportModalComponent` on the board page swept into `9e0d9b54f`) now has its owner commit family above.

### `MWB-T-13` part A — multi-select Category / Funding source / Center in the shared service + Results tab — **PASS** (attempt 1) · 2026-09-05

- **Implementer:** `akili-implementer` on `opus` · skills `angular-developer`, `onecgiar-pr-client:spartan`, `tdd`. Files (6, `programme-results/**` only): `services/programme-results-filter.service.{ts,spec.ts}` (`selectedCategories/Origins/Centers: string[]` replace the three single signals; OR within / AND across; per-value chips + `clearChip`; `clearX(value?)`, `toggleX`; exported `parseListParam`/`joinListParam`/`sameListParam`), `services/programme-results-query-params.ts` (doc), `programme-results.component.{ts,html,spec.ts}` (three `app-pr-filter-multiselect`, option computeds with `computed({ equal: sameFilterOptions })` so the panel survives ticks, `withSelectedOptions`, URL codec, handlers removed). Overview legacy single-value deep links hydrate as one-element arrays and are not rewritten.
- **Verification:** Jest `30 suites / 1224 tests` (programme-results + dashboard-lab), tsc, lint. Browser (SP01 Results): two categories → 87 rows = 61 + 26 (OR), URL `category=a,b`; legacy `?category=Knowledge%20product` → 26 rows, URL untouched; `?center=CIMMYT,IFPRI` → 12 = 11 + 1; panel open across two ticks.
- **Reviewer (`akili-reviewer`, opus):** `STATUS: PASS` — "correct OR-within/AND-across semantics, per-value chips and `clearChip`, an `Other`-bucket-safe option list, and a comma-list URL codec that leaves legacy single-value Overview deep links working and un-rewritten; every prior … case was adapted rather than deleted, and 24 new cases cover the added behaviour."
- **ADVISORY → part B:** update `programme-results/CLAUDE.md` (service contract lines 18/153) in the same commit family; widen the brand checkbox accent selector from `.pgr-filter--section` to `.pgr-filter`; codec comma limitation noted; delete the board's duplicate codec in part B.
- **Gate:** auto-approved (pre-approved mode).

### `MWB-T-14` — Filter chips overflow and multiselect closing — attempt 1 **FAIL** (evidence) · 2026-09-05

- **Implementer (opus):** root cause measured live: `app-pr-filter-multiselect` has no overlay (panel shown via `.field:focus-within`); a click focuses the row checkbox; the board's option computeds rebuilt a fresh array per tick → rows destroyed → focused checkbox detached → panel closed. Fix: `computed({ equal: sameFilterOptions })` on the five option computeds + detached-target guard; chips aggregated per dimension at ≥ 3 values (`MWB_CHIP_SUMMARY_THRESHOLD`), summary chip label opens the popover, × clears the dimension, badge counts values; no `+N more` (measured 2 lines). Jest `7 / 147`, tsc, lint, CT `10 passing`. Live: three centers ticked without closing; chips 14 → 5, 4 lines → 2. Finding: the shared control's grouped mode (`get filteredGroups()` rebuilds objects every CD) makes the Results tab's Section multiselect close too → part B.
- **Reviewer (opus): `STATUS: FAIL`** — (1) the CT "stays open while ticking" case clicks the row centre; Cypress focuses the nearest focusable **ancestor** (the `<a class="field">`), not the child checkbox, so the real failure path (checkbox focused → detached) is never exercised and the case is green with or without the fix (task FAIL input + disqualifier). Remedy: click `input[type="checkbox"]` inside the option, assert focus stays inside the control after the tick, run the FAIL input (drop the `equal` guard → case RED), fix the comment. (2) the "no `+N more`" justification cites a six-chip worst-case CT measurement that does not exist (CT has one 5-chip fixture; `Created by` chip needs `scope: 'all'`; the `Search:` chip has an unbounded label). Remedy: extend the CT with the real worst case (scope All + Created by + long search) at both viewports and keep the ≤ 2-lines assertion, or state precisely what was measured and record the gap.
- **ADVISORY (recorded):** shared control needs `trackBy` on both `*ngFor`s + memoised `filteredGroups` (part B); the detached-target guard is defensive, not load-bearing (same event dispatch); `MyWorkBoardChip.count` unused; rename the "badge equals chips" Jest title.
- **Leader adjudication:** rework attempt 2 (evidence fixes only; behaviour accepted).

#### `MWB-T-14` attempt 2 (evidence fixes) — landed by a foreign commit · 2026-09-05

- **Implementer (opus):** CT tick now clicks the option's `input[type="checkbox"]` (the real defect path; Cypress focuses the nearest focusable *ancestor* of the click point, so the row-centre click had been focusing the trigger anchor) and asserts the clicked node stays connected + focused. **FAIL input executed:** dropping `{ equal: sameFilterOptions }` → RED at both viewports (`the ticked option row survived the tick … expected false to equal true`; with that guard disabled too: `center panel STILL open after the first tick: expected '0' to equal '1'`) → restored → green. Worst-case chip row measured: aggregated fixture (7 chips incl. `Created by` + 39-char search) = **2 lines**; widest reachable case (every multi dimension at two long real values → nothing aggregates, 9 chips) = **3 lines** (tops `[70,110,150]`) at 1280 and 1440 → the doc comment now states the gap; the CT pins the widest case at 3 as a characterisation. `MyWorkBoardChip.count` removed; Jest title corrected. Jest `7 / 149`, tsc, lint, CT `14 passing`.
- **Traceability incident:** while attempt 2 was in progress, a concurrent session committed the whole working tree as `b1ca9ef1f ✨ feat(result-framework-reporting) [SPEC:changes/emerging-result-cta-placement]` — it swept T-14 attempt 1 + 2 (`my-work-board.component.{ts,html,scss,spec.ts}`, `my-work-board.cy.ts`, `services/my-work-board.service.spec.ts`) and this file's attempt-1 FAIL entry. HEAD == disk, nothing lost, but **T-14's code is filed under a foreign spec label**; this entry is the correction for the archive. The T-14 Reviewer of attempt 2 runs on the working-tree files as they stand.
- **Leader adjudication:** the measured 3-line state makes `+N more` a live requirement (task item (1)) → **attempt 3 implements it**; scoped review afterwards covers attempts 2 + 3.

#### `MWB-T-14` attempt 3 (`+N more`) — scoped re-judgment **PASS** → task **PASS** (3 attempts) · 2026-09-05

- **Implementer (opus):** `visibleChipLimit` measured after render (row always measured fully expanded inside one synchronous callback; line membership by vertical centre; pass 1 without the button, pass 2 drops trailing chips until ≤ 2 lines with `Clear filters` reachable); `afterNextRender` + width-guarded `ResizeObserver` + `afterRenderEffect`; hidden chips `.mwb-chip-hidden { display: none !important }` (out of the a11y tree); `+N more` `<button aria-expanded aria-controls>` ↔ `Show less`; resets on scope/phase/`clearAll`. Numbers: CT widest 9 chips → 6 in row, 2 lines, `+3 more`; aggregated 7 → 2 lines, no button; live (sidebar open) 7 → 4, `+3 more`; with a 39-char search 8 → 4, `+4 more`; expanded 3 lines; `documentElement.scrollWidth <= innerWidth` in every state. FAIL inputs: `MWB_CHIP_MAX_LINES=99` → RED (`3 line(s)`) at both viewports; `isChipHidden→false` / `hiddenChipCount→0` → 2 Jest cases RED each. Verification: Jest `7 / 156`, tsc, lint, CT `14 passing`.
- **Reviewer (scoped, opus):** `STATUS: PASS` — "the CT now ticks the checkbox itself and asserts the connected+focused node (with the recorded `equal`-guard FAIL input), and the widest reachable nine-chip row is held to ≤ 2 lines by a real-layout measurement behind `+N more` … The measure/hide cycle is convergent (width-guarded observer + always-measure-expanded), `Clear filters` and the overflow button can never be hidden, chip indexing is stable, and attempt 1's accepted behaviour is untouched."
- **ADVISORY (recorded → `pending-archive.md`):** the `!important` justification names the wrong mechanism (emulated-encapsulation specificity already wins) — correct the comment; pass-2 label width vs final label (add `min-w`); no floor of 1 chip / no narrow many-chip CT case; possible one extra convergent `ResizeObserver` pass; **CT determinism follow-up**: the `MWB-T-11` 390×844 jumper landing assertion races the smooth scroll (~1 in 6) — force `scroll-behavior: auto` in the CT mount or wait for `scrollend`.
- **Gate:** auto-approved (pre-approved mode).

### `MWB-T-13` part B — phase 1 (shared multiselect row stability, accent, folder doc) · 2026-09-05

- **Implementer (opus):** `pr-filter-multiselect.component.{ts,html,spec.ts}` — `trackByOption`/`trackByGroup` on all three `*ngFor`s; lazy memo (`refreshCache()` keyed on group/label/filter/search + content compare) so content-equal `options` keep the same list instances (rows no longer recreated → focused checkbox survives → panel stays open, fixing the Results tab's Section/Category/… multiselects at the source); public API unchanged. `programme-results.component.ts` — brand checkbox accent widened from `.pgr-filter--section` to `.pgr-filter`. `programme-results/CLAUDE.md` — filter-service contract rewritten for part A (`selectedCategories/Origins/Centers`, per-value chips, codec exports), `Verified: 2026-09-05 · fe1d7402e`.
- **Evidence:** TDD RED×5 → green (incl. a false-green caught: DOM nodes compared with `toEqual` → per-node `toBe`); Jest `4 suites / 232 tests`, tsc, lint; browser (SP01/results, fresh bundle): two real CDP ticks on Section and on Category → panel stays open, `activeElement` = the checkbox; brand accent on all four controls.
- **Phase 2 (board consumes the service, local layer deleted): deferred by the user** ("necesito que vamos terminando") — recorded in `tasks.md` (`[~]`) and `pending-archive.md`.
- **Reviewer (`akili-reviewer`, opus): `STATUS: PASS`** — "stable `trackBy` on every `*ngFor`, a memo whose invalidation key and content compare cover exactly the inputs the two lists derive from (search still live), zero change to the consumer-facing API, per-node `toBe` identity proofs in both flat and grouped mode". Leader correction before commit: the folder doc claimed the board already consumes the shared signals (that is the deferred phase 2) → line rewritten to state the board-local copy and the deferral. **Contract note (advisory):** `pr-filter-multiselect` now memoises on `options` reference + content — consumers must rebind `options`, never mutate the bound array in place (no consumer does today). `programme-results/CLAUDE.md` exceeds the 120-line cap (~140) — trim on the next pass.
- **Gate:** auto-approved (pre-approved mode).
