...
- **`MWB-T-13` phase 2 deferred (user, 2026-09-05):** the board still runs its own multi-value layer (`selectedCategories/Origins/Centers` in `my-work-board.service.ts`, local `parseListParam`/`joinListParam`/`withSelectedOptions`) duplicating the shared `ProgrammeResultsFilterService` (part A). Behaviour is identical on both tabs; the follow-up is to bind the board to the service's signals, drop the local layer and codec, and adapt the specs/CT fake (~200 LOC).
