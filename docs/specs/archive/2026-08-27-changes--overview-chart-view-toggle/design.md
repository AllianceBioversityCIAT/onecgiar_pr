...
| `CVT-DD-9` | **Amendment CVT-A-5 (2026-08-27, owner, CVT-T-3 gate):** new pure builders in `program-overview.charts.ts` — `singleBarOption(bars, color)` (horizontal single-series bar: yAxis = names `inverse` + `interval: 0` + abbreviation formatter, xAxis value, value label at bar end via existing text token, bar fill = existing chart token passed resolved by the caller) + `singleBarTable(...)` + `singleBarLinkFromClick(event, bars)` (dataIndex → `bars[i].link`, null-safe). Both bilateral single-series cards bind one `app-pr-viz-chart` each (options+tableModel+chartClick), height computed from row count (min floor). DOM-bars markup, `bilateralCategoryWidth`-style members and their button rows removed. | One library across the Overview (owner). Mirrors the matrix-card host pattern; KZ-SPO-1; jsdom-safe token-name assertions. |
