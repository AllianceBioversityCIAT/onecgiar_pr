...
- **AMENDMENT CVT-A-5 (2026-08-27, owner, at CVT-T-3 HITL gate):** the two remaining single-series DOM-bars cards — "W3/Bilateral results by indicator category" and "Centers with reported W3/bilateral results" — are **converted to ECharts** via `app-pr-viz-chart` (single-series horizontal bar chart each, value label at bar end, same data inputs, same navigation links). Supersedes this spec's "single-series bar cards out of scope" line and the family's "DOM bars for single-series rows" pattern for THESE two cards (owner-directed: one charting library across the Overview). A11y follows the family invariant: `tableModel` on every host, SVG marks non-focusable, the visually-hidden table is the keyboard path (the DOM-button keyboard rows are removed — accepted tradeoff, consistent with the matrix/donut cards). KZ-SPO-1 applies to the category axis (interval: 0 + display abbreviation where labels are long). Heading contract stays 7.
