...
### Addendum (2026-08-31, owner field report): grouped view paints partial numbers that then jump

- **Symptom (SP01 `tocView=aows`):** stats card and AoW headers render values mid-load (TOTAL KPIS 367 → 449; headers "0 of 0 · 0%" → real) instead of a skeleton.
- **Root causes:** (1) the same **not-yet-started ≠ loading** class as the `?kpi=` race — `loading` was `!toc && loadingTocKeys.has(key)`, and before `loadToc` runs the key is not in the set, so the pre-request gap rendered as loaded-and-empty; same gap in `loadingAows` and both bucket cards. `loadToc` ALWAYS caches (an errored fetch caches an empty ToC), so `!toc` alone is a safe loading definition. (2) The stats card and group headers had no loading branch at all.
- **Fixes:** `loading: !toc` (per-AoW groups + both buckets); `loadingAows` = "AoW list not cached"; group headers skeleton the KPI count + ratio while their ToC loads; new `statsLoading` input on the band's stats card (host: `plannedReportingStatsLoading` = AoW list loading OR any group loading) renders pulse figures.
- **Verification:** live trace on SP01 — 4 stats skeletons + 20 header skeletons during load → 4 header skeletons as the last group resolves (stats card still skeleton, correct) → all figures land at once (449), no intermediate jumps. Folder 19 suites / 619 green; tsc + lint clean.
- **Concurrency note:** commits alongside the parallel session's per-section expand/collapse for the By-AOW view (its tests included in the green run). Its rewrite of the band binding had dropped my `[statsLoading]` line mid-flight — re-added; second occurrence of the rebuilt-from-stale-copy hazard (kaizen candidate).
