...
## Post-archive fix — lifecycle timestamps and sweeper cutoffs in DB time (2026-09-16) — **PASS** (attempt 1)

| Field | Value |
|---|---|
| **Trigger** | User report: "AI-assisted processing failed — No AI worker picked up this job before the queue-stall window elapsed" on recent jobs. Leader diagnosis from a read-only `bilateral_ai_jobs` query: app-written timestamps 5 h behind DB-generated `created_date` (consumer on a Bogota laptop, sweeper on prtest/UTC) → false `TIMED_OUT` 48 s after start (`6716bf59…`), blind liveness guard; plus no consumer on prtest (Lambda) → truthful `QUEUE_STALLED` (`f9906dfa…`, `f44de50c…`, `b1a92b43…`, `started_date = NULL`). Recorded as kaizen `L3` and `APF-OQ-7`. |
| **Route** | `/akili-quick` gate failed (logic + regression test) → user chose the pragmatic path: direct fix with Implementer + Reviewer under this spec's slug |
| **Implementer** | `akili-implementer` on **opus** (first sonnet worker killed by the session limit mid-edit at 10:50; resumed on the working tree) · skills `nestjs-expert`, `tdd` · effort high |
| **Reviewer** | `akili-reviewer` on **sonnet** (author ≠ auditor; opus was the author) |

**Files (8, all `onecgiar-pr-server/src/api/bilateral-ai/`, +698/−206):** `bilateral-ai.config.ts` (+ spec: `bilateralAiDbNow` raw `CURRENT_TIMESTAMP` helper), `services/bilateral-ai.service.ts` (+ spec: 8 write sites converted — `attemptStart`, `setStage`, COMPLETED, final-FAILED, retry-requeue, `retryJob` reset, two `QUEUE_NOT_AVAILABLE` completions; `getExpectations` 90-day window in SQL; stray `MoreThan` import fixed), `bilateral-ai-sweeper.cron.ts` (+ spec: attempt-timeout, stall and liveness windows as `DATE_SUB(NOW(), INTERVAL n SECOND)` query-builder conditions; flips write DB time), `services/bilateral-ai-notifications.service.ts` (+ spec: duration via `TIMESTAMPDIFF(SECOND, queue_entry_date, COALESCE(completed_date, NOW()))`, own try/catch, `terminalDate` option removed). Left in JS on purpose: `queue_position` (two DB values), in-process expectations cache TTL.

**Verification:** `npx jest src/api/bilateral-ai src/api/notification --silent --reporters=summary --forceExit` → `Test Suites: 13 passed, Tests: 228 passed, 228 total` · `npx eslint "src/api/bilateral-ai/**/*.ts" --quiet` → exit 0 · `npx tsc --noEmit -p tsconfig.json` → exit 0. Regression cases: `timezone skew regression: lifecycle timestamps are written in DB time, never from the process clock`; sweeper SQL windows asserted from the query builder's actual arguments (`not.toMatch(/\d{4}-\d{2}-\d{2}/)` proves no serialized JS `Date` reaches the query); notifications 90 s → no mail / 360 s → mail from the mocked `TIMESTAMPDIFF`; in-app row never suppressed by a duration-query failure.

**Reviewer `STATUS: PASS`:** all four columns written via `bilateralAiDbNow` at every site (tree grep, not just diff); cutoffs on the right columns and getters (liveness reuses the stall window per `APF-R-2` B); conditional `WHERE status` flips and notify-on-`affected` guards unchanged; `INTERVAL` literals numeric from config → injection-safe by construction; duration read fails open exactly as before (`elapsedSeconds == null || ≥ 120` → mail); two mix classes and `< 5 → null` untouched; no module edit needed (`BilateralAiJob` already in `forFeature`); nothing outside the module changed. **ADVISORY (recorded):** trim the "Was: …" comments in the specs later; `Math.round(Number(env.X)/1000)` on a malformed env yields `NaN SECOND` at query time (pre-existing getter pattern; a startup validation would fail faster — same family as the T-1 config advisory).

**Not fixed here (by decision):** `orm.config.ts` `timezone` stays as is (project-wide change with its own regression sweep); the residual read-path skew on a non-UTC API host (`getJob` serializing `queue_entry_date`, client elapsed) is mitigated by running local API processes with `TZ=UTC` — see kaizen `L3`. TEST consumer topology → `APF-OQ-7`.
