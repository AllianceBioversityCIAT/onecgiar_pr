...
- **`APF-OQ-7`** — **OPEN (post-archive, 2026-09-16): where does the RMQ consumer run on TEST?** §10 assumed "the consumer runs inside the API process (`main.ts`)". On prtest the API is deployed as Lambda (`dist/lambda.handler`), which runs the `@Cron` sweeper but never calls `connectMicroservice`, so no consumer exists there; jobs are consumed only while a developer runs `main.ts` locally. Consequence observed on 2026-09-15/16: three jobs (`b1a92b43…`, `f44de50c…`, `f9906dfa…`) reached `QUEUE_STALLED` after exactly 30 min with `started_date = NULL` — a truthful outcome, not a defect. *Decision needed:* (a) run the consumer + sweeper in a container/worker deployable on TEST (design `APF-DD-2` revisit trigger), or (b) gate the sweeper so it runs only in a process that also consumes (flag set in `main.ts`), leaving Lambda inert. Until decided, `QUEUE_STALLED` on prtest is expected whenever no local consumer is up. Companion fix (same date): lifecycle timestamps and sweeper cutoffs moved to DB time — see kaizen `L3`.
