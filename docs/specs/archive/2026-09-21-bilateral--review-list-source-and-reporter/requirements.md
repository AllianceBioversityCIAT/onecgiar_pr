...
| D7 | Copy drift — a second AI-transparency string | `grep -rn "Generated with AI assistance" onecgiar-pr-client/src` MUST return **7** hits — **unchanged from the pre-spec baseline** — and this spec MUST introduce none of them. Corrected at execute time 2026-09-21: the original "exactly 1" was a miscount. The real baseline is 1 constant definition (`ai-provenance-notice.component.ts:12`) + 1 **pre-existing** hard-coded duplicate in production (`ai-processing-panel.component.html:158`, an APF-R-12 violation predating this spec — out of scope, see `design.md` §13) + 5 spec-file assertions. `BSR-R-5`'s meaning is unchanged: the repository MUST NOT gain a second copy |
