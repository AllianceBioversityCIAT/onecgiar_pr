...
| `OverviewControlsComponent` | standalone, presentational | Phase selector as a **token-styled combobox + ARIA listbox in a CDK overlay** (the SP Overview "Scope" pattern / the Drafts project filter) — options from `PhasesService.phases.reporting` filtered to P25, sorted `phase_year` desc, "Open" badge. **Corrected at `COV-T-8` H-3 (user HITL):** the design originally named `app-pr-select`, which renders the legacy PrimeNG look; the user rejected it on the live page, Filter button + popover (dimensions per `COV-R-3`, option lists passed as inputs), chips, Clear. Emits `phaseChange`, `filtersChange`, `clear`. Escape/outside-click close without applying. |
...
- **Responsive:** grid `grid-cols-1 min-[640px]:grid-cols-2 min-[900px]:grid-cols-3 min-[1280px]:grid-cols-5` for the KPI deck (**corrected at `COV-T-8`:** the original `sm:grid-cols-2` mixed a named breakpoint with arbitrary `min-[…]` ones; Tailwind v4 emits named-breakpoint rules after the arbitrary group, so `sm:` won the cascade at every width ≥ 640 px and the deck was capped at 2 columns — caught by the CT gate, never by Jest. Never mix `sm:/md:/lg:` with `min-[Npx]:` on the same property.); rows A/C `min-[1280px]:grid-cols-[3fr_2fr]` / `min-[1280px]:grid-cols-2`; chart heights fixed per card (`height` input) — 180–220px; chips row `overflow-x-auto no-scrollbar` under 900px.
...
- Patterns reused: `docs/specs/archive/2026-08-28-changes--overview-phase-filter/design.md` (DD-1, DD-4); phase selector look: SP Overview scope control (`program-overview.component.html`) and Drafts project filter (`my-draft-results.component.html`) — not `app-pr-select`, `docs/specs/archive/2026-08-28-changes--portfolio-overview-echarts/`, `pages/result-framework-reporting/pages/bilateral-review/bilateral-review.query-params.ts`
