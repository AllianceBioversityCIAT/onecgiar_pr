...
review tab
  ...
  └── Cycle select (re-pick is a no-op via setPhase() + writeValue re-sync — §6.2 correction) → ?phase= (replaceUrl) → list effect re-fetches; allExpanded=true + nonce bump; search/status/centers untouched
```
...
- `currentPhaseId` computed: reads `dataControlSE.reportingPhaseVersion()` then normalizes `reportingCurrentPhase.phaseId` through one exported helper that rejects `null`/`undefined`/`''` **before** `Number()` (the shell cold-boots with `phaseId: null`, and `Number(null) === 0`) and `<= 0` after; when the shell is unresolved but the catalog has settled, falls back to the portfolio-filtered catalog row with `status === true` (same fact as `GET_versioning(OPEN, REPORTING)`) — execution correction, BRC-T-1 (same tracking trick as `dashboard-lab.effectiveVersionId`, `:1477-1486`; bigint-string normalization as `:1509-1516`).
...
- `centerStrip` computed over `searchFiltered`: `{ code, acronym, pending }` per distinct `lead_center` (acronym → code via existing `acronymToCode`, falling back to the acronym when the catalog lacks it — same as the popover option), sorted pending desc, acronym asc, plus a trailing **Not specified** bucket for blank `lead_center` whose `code` is the param-safe sentinel `__unassigned__` (**execution correction, BRC-T-2:** an empty code cannot round-trip through the `?center=` csv, so the chip never stayed pressed; the page maps the sentinel back to a blank `lead_center` when filtering rows); `allPending` = Σ (equals KPI Pending by construction). Pressed logic per BRC-R-2 from `centers()`.
- `phaseIndicator` computed: `selectedVersionId !== currentPhaseId` → phase name string.

### 6.2 Components

**`BilateralReviewCenterStripComponent`** (standalone, OnPush): inputs `items: {code, acronym, pending}[]`, `allPending`, `selectedCodes: string[]`, `maxVisible = 12`; output `selectCenter(code | null)` (**execution correction, BRC-T-2:** `select` is a native DOM event name, forbidden by the enforced `@angular-eslint/no-output-native`; contract unchanged). Renders `role="group"` with the same chip classes as the status chips (copy them from `bilateral-review.component.html:235-296`), `aria-pressed` per BRC-R-2, accessible name "IITA, 100 pending"; tail beyond `maxVisible` behind a "+N more" chip toggling an `expanded` signal (BRC-R-21). No native `disabled`.

**Popover Cycle select**: `app-pr-filter-select` (single; `options`, `optionLabel`, `optionValue`, `emptyValue`, `(changed)` — `shared/components/pr-filter-select/pr-filter-select.component.ts:33-48`) — same component the band uses for AoW (`reporting-program-band.component.html:427-433`) — options from `knownPhases` (label = `phase_name`), value = selected id via `[ngModel]`. **Correction (execution, BRC-T-1):** do **not** bind `[emptyValue]` to the selected id — the component's `hasValue` getter treats `value === emptyValue` as "nothing selected" and would always render the placeholder (contradicts AC-6). Instead the default `emptyValue` stays, and `setPhase()` treats a non-numeric emit (the component's own re-pick-to-`emptyValue`) as a no-op and re-syncs the select through `writeValue(selectedVersionId())` so the phase name stays visible (R-7/AC-8b); `(changed)` writes `?phase=` only when the value differs.
...
Tailwind-first; chips reuse the status-chip classes (`bilateral-review.component.html:235-296`); the "+N more" chip carries no icon (**execution correction, BRC-T-2:** the client is lucide-only, so the `material-icons-round` caret originally named here is dropped); tokens per design.md §7. Strip wraps (`flex-wrap`) and lives inside `#workArea` under the status chips; CT asserts no body overflow.
