...
- **Status:** in-progress — pivot resolved 2026-08-26 (see `execution.md` § Pivot Record: RES-T-1); code + Reviewer PASS complete, awaiting user's manual browser check (`RES-TEST-2`, hover-only) before `[x]`.
...
- **Definition of done:**
  ...
  - [ ] **Manual browser check (required — the class of defect a jsdom unit test cannot see):** run `npm start`, open the Reporting tab for a program with Intermediate Outcomes and at least one AoW with HLO/Outcome indicators (e.g. `SP02`), confirm by **mouse hover only** (Tab-focus is no longer expected to trigger the tooltip — see above):
    - Intermediate outcomes card → tooltip shows the exact copy on hover.
    - An AoW card → no tooltip on Target (existing `achievedTooltip` on the Achieved cell one column over is unaffected — confirm it still shows, to catch an accidental copy-paste onto the wrong cell).
    - **Disqualifying input for this check:** if the tooltip also appears on an AoW card's Target cell, or does not appear at all on an Intermediate row after a hard refresh (confirm you're not viewing a stale `ng serve` bundle per the client `CLAUDE.md` "Verifying in a REAL browser" §9 trap #2), the task is NOT done — do not mark this box checked on a partial or ambiguous observation.
...
### `RES-T-2` — Show the tooltip on AoW-card Outcomes rows that are also Intermediate Outcomes *(added 2026-08-26, scope amendment; mechanism revised same day after live-data verification)*
