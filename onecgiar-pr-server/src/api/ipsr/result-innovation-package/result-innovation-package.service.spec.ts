...
describe('ResultInnovationPackageService', () => {
  ...
  const repo = () => ({ find: jest.fn(), findOne: jest.fn(), findOneBy: jest.fn(), save: jest.fn(), update: jest.fn(), createQueryBuilder: jest.fn() });

  const mockResultRepository = { ...repo(), getResultById: jest.fn(), getLastResultCode: jest.fn() } as any as jest.Mocked<ResultRepository>;
  const mockVersionsService = { $_findActivePhase: jest.fn() } as any as jest.Mocked<VersionsService>;
  const mockResultByInitiativeRepository = { findOne: jest.fn(), InitiativeByResult: jest.fn(), save: jest.fn() } as any as jest.Mocked<ResultByInitiativesRepository>;
  const mockRegionRepository = { save: jest.fn() } as any as jest.Mocked<ResultRegionRepository>;
  const mockCountryRepository = { save: jest.fn(), findOne: jest.fn() } as any as jest.Mocked<ResultCountryRepository>;
  const mockIpsrRepository = { save: jest.fn(), find: jest.fn() } as any as jest.Mocked<IpsrRepository>;
  const mockRIPRepository = { save: jest.fn() } as any as jest.Mocked<ResultInnovationPackageRepository>;
  const mockIpAARepo = { mapActionAreaOutcome: jest.fn(), save: jest.fn() } as any as jest.Mocked<ResultIpAAOutcomeRepository>;
  const mockClarisaAARepo = { find: jest.fn() } as any as jest.Mocked<ClarisaActionAreaOutcomeRepository>;
  const mockImpactIndicatorRepo = { mapImpactAreaOutcomeToc: jest.fn() } as any as jest.Mocked<ResultsImpactAreaIndicatorRepository>;
  const mockImpactAreaRepo = { save: jest.fn() } as any as jest.Mocked<ResultIpImpactAreaRepository>;
  const mockActiveBackstoppingRepo = { find: jest.fn() } as any as jest.Mocked<ActiveBackstoppingRepository>;
  const mockConsensusWPRepo = { find: jest.fn() } as any as jest.Mocked<consensusInitiativeWorkPackageRepository>;
  const mockRegionalIntegratedRepo = { find: jest.fn() } as any as jest.Mocked<RegionalIntegratedRepository>;
  const mockRegionalLeadershipRepo = { find: jest.fn() } as any as jest.Mocked<RegionalLeadershipRepository>;
  const mockRelevantCountryRepo = { find: jest.fn() } as any as jest.Mocked<RelevantCountryRepository>;
  const mockByEvidencesRepo = {} as any as jest.Mocked<ResultByEvidencesRepository>;
  const mockByInstRepo = {} as any as jest.Mocked<ResultByIntitutionsRepository>;
  const mockByInstTypeRepo = {} as any as jest.Mocked<ResultByIntitutionsTypeRepository>;
  const mockValidationRepo = {} as any as jest.Mocked<resultValidationRepository>;
  const mockSdgRepo = { mapSdgsToc: jest.fn(), save: jest.fn() } as any as jest.Mocked<ResultIpSdgTargetRepository>;
  const mockInitiativeBudgetRepo = { save: jest.fn() } as any as jest.Mocked<ResultInitiativeBudgetRepository>;
  ...
  const mockYearRepository = { findOne: jest.fn() } as any as jest.Mocked<YearRepository>;
  ...
  const mockDiscontinuedRepo = { inactiveData: jest.fn(), findOne: jest.fn(), update: jest.fn(), save: jest.fn() } as any as jest.Mocked<ResultsInvestmentDiscontinuedOptionRepository>;
  const mockVersioningService = { $_findActivePhase: jest.fn() } as any as jest.Mocked<VersioningService>;
  const mockResultCountrySubnationalRepository = { find: jest.fn(), save: jest.fn() } as any as jest.Mocked<ResultCountrySubnationalRepository>;
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: ResultByInitiativesRepository, useValue: mockResultByInitiativeRepository },
        ...
        { provide: ResultInnovationPackageRepository, useValue: mockRIPRepository },
        { provide: ResultIpAAOutcomeRepository, useValue: mockIpAARepo },
        { provide: ClarisaActionAreaOutcomeRepository, useValue: mockClarisaAARepo },
        { provide: ResultsImpactAreaIndicatorRepository, useValue: mockImpactIndicatorRepo },
        { provide: ResultIpImpactAreaRepository, useValue: mockImpactAreaRepo },
        { provide: ActiveBackstoppingRepository, useValue: mockActiveBackstoppingRepo },
        { provide: consensusInitiativeWorkPackageRepository, useValue: mockConsensusWPRepo },
        { provide: RegionalIntegratedRepository, useValue: mockRegionalIntegratedRepo },
        { provide: RegionalLeadershipRepository, useValue: mockRegionalLeadershipRepo },
        { provide: RelevantCountryRepository, useValue: mockRelevantCountryRepo },
        ...
        { provide: ResultByIntitutionsTypeRepository, useValue: mockByInstTypeRepo },
        { provide: resultValidationRepository, useValue: mockValidationRepo },
        { provide: ResultIpSdgTargetRepository, useValue: mockSdgRepo },
        { provide: ResultInitiativeBudgetRepository, useValue: mockInitiativeBudgetRepo },
        ...
        { provide: ResultsInvestmentDiscontinuedOptionRepository, useValue: mockDiscontinuedRepo },
        { provide: VersioningService, useValue: mockVersioningService },
        { provide: ResultCountrySubnationalRepository, useValue: mockResultCountrySubnationalRepository },
      ],
    }).compile();
  ...
  it('findRegionalIntegrated returns OK', async () => {
    (mockRegionalIntegratedRepo.find as jest.Mock).mockResolvedValueOnce([{ id: 1 }]);
  ...
  describe('createHeader validations', () => {
    ...
    const dtoBase: any = { result_id: 2, initiative_id: 3, geo_scope_id: 2, regions: [{ id: 1, name: 'X' }], countries: [] };

    it('returns not found when core result missing', async () => {
      (mockResultRepository.getResultById as jest.Mock).mockResolvedValueOnce(null);
    ...
    it('returns bad request when initiative is not mapped to core', async () => {
      (mockResultRepository.getResultById as jest.Mock).mockResolvedValueOnce({ id: '2', result_type_id: 7, title: 'Core.', description: '' });
      (mockResultByInitiativeRepository.findOne as jest.Mock).mockResolvedValueOnce({});
      (mockResultByInitiativeRepository.InitiativeByResult as jest.Mock).mockResolvedValueOnce([{ inititiative_id: 99 }]);
      const res = await service.createHeader(dtoBase, user);
    ...
    it('returns bad request when initiative_id missing', async () => {
      (mockResultRepository.getResultById as jest.Mock).mockResolvedValueOnce({ id: '2', result_type_id: 7, title: 'Core.', description: '' });
      (mockResultByInitiativeRepository.findOne as jest.Mock).mockResolvedValueOnce({});
      (mockResultByInitiativeRepository.InitiativeByResult as jest.Mock).mockResolvedValueOnce([{ inititiative_id: 3 }]);
      const res = await service.createHeader({ ...dtoBase, initiative_id: null }, user);
      expect(res.status).toBe(HttpStatus.BAD_REQUEST);
    });
    ...
    it('returns bad request when geo_scope_id missing', async () => {
      ...
      (mockResultByInitiativeRepository.findOne as jest.Mock).mockResolvedValueOnce({});
      (mockResultByInitiativeRepository.InitiativeByResult as jest.Mock).mockResolvedValueOnce([{ inititiative_id: 3 }]);
      const res = await service.createHeader({ ...dtoBase, geo_scope_id: null }, user);
      expect(res.status).toBe(HttpStatus.BAD_REQUEST);
    });

    it('returns bad request when result type is not innovation development', async () => {
      (mockResultRepository.getResultById as jest.Mock).mockResolvedValueOnce({ id: '2', result_type_id: 6, title: 'Core.', description: '' });
      (mockResultByInitiativeRepository.findOne as jest.Mock).mockResolvedValueOnce({});
      (mockResultByInitiativeRepository.InitiativeByResult as jest.Mock).mockResolvedValueOnce([{ inititiative_id: 3 }]);
      const res = await service.createHeader(dtoBase, user);
    ...
    it('returns bad request when title already exists', async () => {
      (mockResultRepository.getResultById as jest.Mock).mockResolvedValue({ id: '2', result_type_id: 7, title: 'Core.', description: '' });
      (mockResultByInitiativeRepository.findOne as jest.Mock).mockResolvedValue({});
      (mockResultByInitiativeRepository.InitiativeByResult as jest.Mock).mockResolvedValue([{ inititiative_id: 3 }]);
      (mockVersioningService.$_findActivePhase as jest.Mock).mockResolvedValue({ id: 1 });
      const qb = { where: jest.fn().mockReturnThis(), andWhere: jest.fn().mockReturnThis(), getMany: jest.fn().mockResolvedValue([ { id: 10, result_code: 101 } ]) };
      (mockResultRepository.createQueryBuilder as jest.Mock).mockReturnValue(qb as any);
      const res = await service.createHeader(dtoBase, user);
      expect(res.status).toBe(HttpStatus.BAD_REQUEST);
    ...
    it('creates header successfully', async () => {
      (mockResultRepository.getResultById as jest.Mock).mockResolvedValue({ id: '2', result_type_id: 7, title: 'Core.', description: '', gender_tag_level_id: null, climate_change_tag_level_id: null, lead_contact_person: null });
      (mockResultByInitiativeRepository.findOne as jest.Mock).mockResolvedValue({});
      (mockResultByInitiativeRepository.InitiativeByResult as jest.Mock).mockResolvedValue([{ inititiative_id: 3 }]);
      (mockVersioningService.$_findActivePhase as jest.Mock).mockResolvedValue({ id: 1 });
      const qb = { where: jest.fn().mockReturnThis(), andWhere: jest.fn().mockReturnThis(), getMany: jest.fn().mockResolvedValue([]) };
      (mockResultRepository.createQueryBuilder as jest.Mock).mockReturnValue(qb as any);
      (mockYearRepository.findOne as jest.Mock).mockResolvedValue({ year: 2024 });
      (mockResultRepository.getLastResultCode as jest.Mock).mockResolvedValue(1000);
      (mockResultRepository.save as jest.Mock).mockResolvedValue({ id: 200 });
      (mockResultByInitiativeRepository.save as jest.Mock).mockResolvedValue({ id: 300 });
      (mockInitiativeBudgetRepo.save as jest.Mock).mockResolvedValue({});
      (mockRIPRepository.save as jest.Mock).mockResolvedValue({});
      (mockIpsrRepository.save as jest.Mock).mockResolvedValue({ result_by_innovation_package_id: 400 });
      (mockIpAARepo.mapActionAreaOutcome as jest.Mock).mockResolvedValue([]);
      (mockClarisaAARepo.find as jest.Mock).mockResolvedValue([]);
      (mockImpactIndicatorRepo.mapImpactAreaOutcomeToc as jest.Mock).mockResolvedValue([]);
  ...
  describe('defaultRelevantCountry', () => {
    ...
    it('returns 3 when geoscope 3/4 and country exists', async () => {
      (mockCountryRepository.findOne as jest.Mock).mockResolvedValueOnce({ id: 1 });
