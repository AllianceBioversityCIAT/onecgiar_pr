...
export class InnovationPathwayStepOneService {
  ...
  async updateMain(
    ...
    try {
      ...
      const geoScope = await this.geoScope(
        result,
        version,
        UpdateInnovationPathwayDto,
        user,
  ...
  async geoScope(
    ...
    try {
      ...
      if (UpdateInnovationPathwayDto.geo_scope_id !== 2) {
        ...
        if (regions) {
          await this._resultRegionRepository.updateRegions(
            id,
            UpdateInnovationPathwayDto.regions.map((r) => r.id),
          );
          if (regions?.length) {
            for (let i = 0; i < regions.length; i++) {
              ...
              updateRegions = await this._resultRegionRepository.save(
                resultRegions,
              );
            }
          }
      ...
      if (
        UpdateInnovationPathwayDto.geo_scope_id === 1 ||
        UpdateInnovationPathwayDto.geo_scope_id === 2
      ) {
        await this._resultCountryRepository.updateCountries(id, []);
      } else if (
        UpdateInnovationPathwayDto.geo_scope_id === 3 ||
        UpdateInnovationPathwayDto.geo_scope_id === 4
      ) {
        await this._resultCountryRepository.updateCountries(
          id,
          UpdateInnovationPathwayDto.countries.map((c) => c.id),
        );
        ...
        await this._resultCountriesSubNationalRepository.inactiveAllIds(
          inactiveCountries.map((el) => el.result_country_id),
        );
        if (countries?.length) {
          for (const ct of countries) {
            ...
            await this.saveSubNational(
              countriExist
                ? countriExist.result_country_id
                : rce.result_country_id,
      ...
      const ipsrResult = await this._innovationByResultRepository.findOneBy({
        result_innovation_package_id: id,
      });

      const coreResult = await this._resultRepository.findOneBy({
        id: ipsrResult.result_id,
      });
