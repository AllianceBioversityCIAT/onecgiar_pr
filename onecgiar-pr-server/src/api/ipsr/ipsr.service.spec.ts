...
describe('IpsrService', () => {
  ...
  describe('findAllInnovations', () => {
    it('returns innovations with OK', async () => {
      (mockIpsrRepository.getResultsInnovation as jest.Mock).mockResolvedValueOnce([
        { id: 1 },
      ]);
      const res = await service.findAllInnovations([1, 2]);
      expect(mockIpsrRepository.getResultsInnovation).toHaveBeenCalledWith([1, 2]);
      expect(res.statusCode).toBe(HttpStatus.OK);
      expect(res.response[0].id).toBe(1);
    });

    it('handles errors via ReturnResponse', async () => {
      (mockIpsrRepository.getResultsInnovation as jest.Mock).mockRejectedValueOnce(
        new Error('boom'),
      );
  ...
  describe('findInnovationDetail', () => {
    it('returns detail with OK', async () => {
      (mockIpsrRepository.getResultInnovationDetail as jest.Mock).mockResolvedValueOnce({ id: 10 });
      const res = await service.findInnovationDetail(10);
      expect(mockIpsrRepository.getResultInnovationDetail).toHaveBeenCalledWith(10);
      expect(res.status).toBe(HttpStatus.OK);
      expect(res.response.id).toBe(10);
    });

    it('returns error when not found', async () => {
      (mockIpsrRepository.getResultInnovationDetail as jest.Mock).mockResolvedValueOnce(null);
  ...
  describe('findOneInnovation', () => {
    it('returns innovation with ad user and discontinued options', async () => {
      (mockIpsrRepository.getResultInnovationById as jest.Mock).mockResolvedValueOnce([
        { id: 5, lead_contact_person_id: 99 },
      ]);
      (mockDiscontinuedRepo.find as jest.Mock).mockResolvedValueOnce([{ opt: 1 }]);
      (mockAdUserRepository.findOne as jest.Mock).mockResolvedValueOnce({ id: 99, mail: 'a@b.c' });

      const res = await service.findOneInnovation(5);
      expect(mockIpsrRepository.getResultInnovationById).toHaveBeenCalledWith(5);
      expect(res.status).toBe(HttpStatus.OK);
      expect(res.response.discontinued_options).toEqual([{ opt: 1 }]);
      expect(res.response.lead_contact_person_data).toEqual({ id: 99, mail: 'a@b.c' });
    });
  ...
  describe('allInnovationPackages', () => {
    it('maps entity and user initiatives', async () => {
      (mockIpsrRepository.getAllInnovationPackages as jest.Mock).mockResolvedValueOnce([
        { initiative_id: 1, name: 'pkg' },
      ]);
  ...
  describe('getIpsrList', () => {
    it('returns list with OK', async () => {
      (mockIpsrRepository.getIpsrList as jest.Mock).mockResolvedValueOnce([{ id: 1 }]);
