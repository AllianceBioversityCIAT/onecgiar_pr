...
export class TypeOneReportRepository {
  ...
  async getFactSheetByInit(
    ...
    SELECT
      GROUP_CONCAT(DISTINCT crc.cgiar_name  SEPARATOR '; ') AS name,
      rbi.inititiative_id AS initiative_id
    FROM 
      prdb.result_region rr 
      LEFT JOIN prdb.clarisa_regions cr ON cr.um49Code = rr.result_region_id
      LEFT JOIN prdb.clarisa_regions_cgiar crc ON crc.un_code = cr.um49Code
