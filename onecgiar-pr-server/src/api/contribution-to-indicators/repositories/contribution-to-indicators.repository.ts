  ...
  private _flattenedResultsQuery(tocId: string) {
    return `
      ...
      from (
        ...
        right join Integration_information.toc_results outcomes on tri.toc_results_id = outcomes.id
        ...
        left join prdb.contribution_to_indicator_results main_ctir on main_ctir.result_id = main_r.id 
        ...
        left join prdb.results_by_inititiative main_rbi on main_rbi.result_id = main_r.id 
        ...
        left join prdb.linked_result lr on lr.is_active and lr.origin_result_id = original_results.result_id
