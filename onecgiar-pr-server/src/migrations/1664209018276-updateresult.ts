...
export class updateresult1664209018276 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`evidence_types\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`name\` varchar(100) NULL, \`description\` text NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`initiative_roles\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`name\` varchar(45) NULL, \`description\` varchar(45) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `ALTER TABLE \`results_by_evidence\` ADD CONSTRAINT \`FK_bbd96dc95ae93743a0995c37df6\` FOREIGN KEY (\`evidence_types_id\`) REFERENCES \`evidence_types\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
