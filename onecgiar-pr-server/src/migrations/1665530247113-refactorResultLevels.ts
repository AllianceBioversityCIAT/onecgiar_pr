...
export class refactorResultLevels1665530247113 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `INSERT INTO \`result_type\` (name,description,result_level_id) VALUES ('Impact Contribution','',?)`,
      [resultLevelImpact.insertId],
    );

    const resultLevelOutcome = await queryRunner.query(
      `INSERT INTO \`result_level\` (name, description) VALUES ('Outcome', 'A change in knowledge, skills, attitudes and/or relationships, which manifests as a change in behavior in particular actors, to which research outputs and related activities have contributed.')`,
    );
    await queryRunner.query(
      `INSERT INTO \`result_type\` (name,description,result_level_id) VALUES ('Policy Change','',?)`,
      [resultLevelOutcome.insertId],
    );
    await queryRunner.query(
      `INSERT INTO \`result_type\` (name,description,result_level_id) VALUES ('Innovation use','',?)`,
      [resultLevelOutcome.insertId],
    );
    await queryRunner.query(
      `INSERT INTO \`result_type\` (name,description,result_level_id) VALUES ('Organizational change','',?)`,
      [resultLevelOutcome.insertId],
    );
    await queryRunner.query(
      `INSERT INTO \`result_type\` (name,description,result_level_id) VALUES ('Other outcome','',?)`,
      [resultLevelOutcome.insertId],
    );

    const resultLevelOutput = await queryRunner.query(
      `INSERT INTO \`result_level\` (name, description) VALUES ('Output', 'Knowledge, or a technical or institutional advancement produced by CGIAR research, engagement and/or capacity development activities.')`,
    );
    await queryRunner.query(
      `INSERT INTO \`result_type\` (name,description,result_level_id) VALUES ('Capacity Sharing for Development','',?)`,
      [resultLevelOutput.insertId],
    );
    await queryRunner.query(
      `INSERT INTO \`result_type\` (name,description,result_level_id) VALUES ('Knowledge Product','',?)`,
      [resultLevelOutput.insertId],
    );
    await queryRunner.query(
      `INSERT INTO \`result_type\` (name,description,result_level_id) VALUES ('Innovation Developmen','',?)`,
      [resultLevelOutput.insertId],
    );
    await queryRunner.query(
      `INSERT INTO \`result_type\` (name,description,result_level_id) VALUES ('Other output','',?)`,
      [resultLevelOutput.insertId],
    );
