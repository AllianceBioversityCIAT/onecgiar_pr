  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
            UPDATE
                result_indicators_targets rit
            SET
                contributing_indicator = target_progress_narrative
            WHERE
                target_progress_narrative REGEXP '[a-zA-Z]|\[^a-zA-Z0-9]';
        `);
    ...
    await queryRunner.query(`
        UPDATE
            result_indicators_targets rit
        SET
            contributing_indicator_tmp = target_progress_narrative
        WHERE
            target_progress_narrative IS NOT NULL;
        `);
