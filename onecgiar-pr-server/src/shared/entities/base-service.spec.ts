import { BaseEntity } from './base-entity';
import { BaseServiceSimple } from './base-service';
import { SetAutitEnum } from '../utils/current-user.util';

class DummyEntity extends BaseEntity {
  id!: number;
  result_id!: number;
  role_id?: number;
  name?: string;
  number?: number;
  unit?: string;
  description?: string;
  other?: string;
}

class DummyService extends BaseServiceSimple<DummyEntity, any> {
  constructor(repo: any, currentUser: any, roleKey: string | null = null) {
    super(DummyEntity, repo, 'result_id' as any, currentUser, roleKey as any);
  }
}

describe('BaseServiceSimple / BaseDeleteService', () => {
  const makeRepo = () => ({
    metadata: { primaryColumns: [{ propertyName: 'id' }] },
    find: jest.fn(),
    update: jest.fn(),
    save: jest.fn(),
  });

  const makeCurrentUser = () => ({
    audit: jest.fn(() => ({ created_by: 7, last_updated_by: 7 })),
  });

  describe('transformArrayToSaveObject', () => {
    it('debe mapear a objetos usando primaryKey del metadata', () => {
      const repo = makeRepo();
      const currentUser = makeCurrentUser();
      const service = new DummyService(repo, currentUser);

      expect(service.transformArrayToSaveObject([1, 2], { result_id: 10 } as any))
        .toEqual([
          { id: 1, result_id: 10 },
          { id: 2, result_id: 10 },
        ]);
    });
  });

  describe('find', () => {
    it('debe construir where con resultKey e is_active=true', async () => {
      const repo = makeRepo();
      repo.find.mockResolvedValue([{ id: 1 }]);
      const currentUser = makeCurrentUser();
      const service = new DummyService(repo, currentUser, 'role_id');

      await service.find([10, 11] as any, 2 as any, { rel: true } as any);

      expect(repo.find).toHaveBeenCalledTimes(1);
      const args = repo.find.mock.calls[0][0];
      expect(args).toEqual(
        expect.objectContaining({
          where: expect.objectContaining({
            is_active: true,
          }),
          relations: { rel: true },
        }),
      );
      expect(args.where.result_id).toBeDefined(); // FindOperator(In)
      expect(args.where.role_id).toBe(2);
    });
  });

  describe('unsetMultiplesPrimary', () => {
    it('debe desmarcar todos si hay más de uno is_primary=true', () => {
      const repo = makeRepo();
      const currentUser = makeCurrentUser();
      const service = new DummyService(repo, currentUser);

      const input = [
        { is_primary: true },
        { is_primary: true },
        { is_primary: false },
      ];
      const res = (service as any).unsetMultiplesPrimary(input);
      expect(res.every((x: any) => x.is_primary === false)).toBe(true);
    });
  });

  describe('create', () => {
    it('debe upsert por generalCompareKey, inactivar otros y agregar auditoría', async () => {
      const repo = makeRepo();
      const currentUser = makeCurrentUser();
      const service = new DummyService(repo, currentUser, 'role_id');

      repo.find.mockResolvedValue([
        {
          id: 1,
          result_id: 10,
          role_id: 2,
          name: 'A',
          other: 'backend',
          is_active: true,
        },
      ]);
      repo.update.mockResolvedValue(undefined);
      repo.save.mockImplementation(async (items: any[]) => [
        { ...items[0], is_active: true },
        { ...items[0], id: 999, is_active: false },
      ]);

      const res = await service.create(
        10,
        [
          { id: 1, name: 'A', other: 'client' },
          { id: 2, name: '' }, // filtrado por isEmpty
        ],
        'name' as any,
        2 as any,
        undefined,
        ['other'] as any,
      );

      // update para inactivar otros
      expect(repo.update).toHaveBeenCalledTimes(1);
      expect(repo.update.mock.calls[0][1]).toEqual(
        expect.objectContaining({ is_active: false }),
      );

      // save con auditoría BOTH
      expect(currentUser.audit).toHaveBeenCalledWith(SetAutitEnum.BOTH);
      expect(repo.save).toHaveBeenCalledTimes(1);
      const savedArg = repo.save.mock.calls[0][0];
      expect(savedArg[0]).toEqual(
        expect.objectContaining({
          created_by: 7,
          last_updated_by: 7,
          result_id: 10,
          role_id: 2,
          name: 'A',
          other: 'client',
        }),
      );

      // retorna solo activos
      expect(res).toHaveLength(1);
      expect(res[0]).toEqual(expect.objectContaining({ is_active: true }));
    });
  });

  describe('upsertByCompositeKeys', () => {
    it('si dataToSave está vacío, debe inactivar todo y retornar []', async () => {
      const repo = makeRepo();
      const currentUser = makeCurrentUser();
      const service = new DummyService(repo, currentUser, 'role_id');

      repo.update.mockResolvedValue(undefined);

      const res = await service.upsertByCompositeKeys(
        10,
        [],
        ['number', 'unit', 'description'] as any,
        2 as any,
      );

      expect(res).toEqual([]);
      expect(repo.update).toHaveBeenCalledTimes(1);
      expect(repo.update.mock.calls[0][0]).toEqual(
        expect.objectContaining({
          result_id: 10,
          is_active: true,
          role_id: 2,
        }),
      );
    });

    it('debe reutilizar existentes por compositeKey y desactivar los no presentes', async () => {
      const repo = makeRepo();
      const currentUser = makeCurrentUser();
      const service = new DummyService(repo, currentUser);

      repo.find.mockResolvedValue([
        {
          id: 1,
          result_id: 10,
          number: 1,
          unit: 'kg',
          description: 'a',
          is_active: true,
        },
        {
          id: 2,
          result_id: 10,
          number: 2,
          unit: 'kg',
          description: 'b',
          is_active: true,
        },
      ]);
      repo.update.mockResolvedValue(undefined);
      repo.save.mockImplementation(async (items: any[]) => items);

      const res = await service.upsertByCompositeKeys(
        10,
        [{ number: 1, unit: 'kg', description: 'a' }],
        ['number', 'unit', 'description'] as any,
      );

      // una actualización para desactivar idsToDeactivate (id=2)
      expect(repo.update).toHaveBeenCalledTimes(1);
      expect(repo.save).toHaveBeenCalledTimes(1);
      expect(currentUser.audit).toHaveBeenCalledWith(SetAutitEnum.BOTH);

      expect(res).toHaveLength(1);
      expect(res[0]).toEqual(expect.objectContaining({ id: 1, is_active: true }));
    });
  });
});

