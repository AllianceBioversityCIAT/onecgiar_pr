...
- **Backend** (`onecgiar-pr-server/.env`): `PORT`, `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER_NAME`, `DB_USER_PASS`, `JWT_SKEY`, `JWT_EXPIRES`, `CGSPACE_DISCOVERY_URL` (CGSpace DSpace 7 REST base; defaults to `https://cgspace.cgiar.org/server/api` when unset), plus RabbitMQ, AWS, Cognito, AD, CLARISA, MQAP, SharePoint vars.
