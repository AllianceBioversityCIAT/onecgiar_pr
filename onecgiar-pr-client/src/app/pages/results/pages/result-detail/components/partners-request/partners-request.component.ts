...
export class PartnersRequestComponent {
  ...
  onRequestPartner(ipsr?: boolean) {
    ...
    const {
      ...
      initiative_name,
      initiative_id
    }: any = this.ipsrDataControlSE.inIpsr
      ? this.ipsrDataControlSE.detailData
      : this.api?.dataControlSE?.currentResult || {};
    //(this.ipsrDataControlSE.detailData);
    const initiativeFinded = this.api.rolesSE.roles.initiative.find(
      initiative => (initiative.initiative_id = initiative_id)
    );
    ...
    Result: [Role: ${initiativeFinded?.description}] - (Code:  ${
      this.ipsrDataControlSE.inIpsr
        ? this.ipsrDataControlSE.detailData?.result_code
        : this.api.resultsSE?.currentResultCode
    }) - ${
      this.ipsrDataControlSE.inIpsr
        ? this.ipsrDataControlSE.detailData?.title
        : this.api.dataControlSE?.currentResult?.title
    },
