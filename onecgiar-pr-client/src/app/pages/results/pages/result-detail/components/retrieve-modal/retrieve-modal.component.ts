...
export class RetrieveModalComponent implements OnInit {
  ...
  onRequestRetrieve() {
    ...
    this.retrieveModalSE.retrieveRequestBody.result_level_id =
      this.resultLevelSE.resultBody.result_level_id;
    this.retrieveModalSE.retrieveRequestBody.result_type_id =
      this.resultLevelSE.resultBody.result_type_id;
    //? get result type
    // this.getResultType();
    //(this.retrieveModalSE.retrieveRequestBody);
    this.api.resultsSE
      .POST_updateRequest(this.retrieveModalSE.retrieveRequestBody)
      .subscribe(
        resp => {
          ...
          this.router.navigate([
            `/result/result-detail/${resp?.response?.newResultHeader?.result_code}/general-information`
          ]);
        },
        err => {
          console.error(err);
