    ...
    list.map(item => {
      let count = () => {
        if (
          ...
          !item.hasOwnProperty('institutions_id')
        )
          return 0;
        return typeof objectCounter[item?.institutions_type_id]?.count ==
          'number'
          ? objectCounter[item?.institutions_type_id]?.count + 1
          : 1;
      };
      //(objectCounter[item?.institutions_type_id]?.count);
      objectCounter[item?.institutions_type_id] = {
        count_name: `${item?.institutions_type_name} (${count()})`,
        count: count()
      };
    });
