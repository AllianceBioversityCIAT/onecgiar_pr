...
export class CPMappedResultsModalComponent {
  @Input() activeTab?: any = {};
  ...
  validateOrder(columnAttr) {
    setTimeout(() => {
      ...
      this.combine =
        !mappedResultTableHTML.querySelectorAll('th[aria-sort="descending"]').length &&
        !mappedResultTableHTML.querySelectorAll('th[aria-sort="ascending"]').length;
