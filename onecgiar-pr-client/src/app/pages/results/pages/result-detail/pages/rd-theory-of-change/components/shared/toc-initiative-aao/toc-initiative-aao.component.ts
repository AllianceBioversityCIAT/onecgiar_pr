...
@Component({
  ...
  imports: [CommonModule, PrFieldHeaderComponent]
})
export class TocInitiativeAaoComponent implements OnInit {
  ...
  getInformatioActionAreaResult() {
    this.api.resultsSE
      .GET_resultActionArea(
        this.initiative?.result_toc_results?.[0]?.results_id,
        this.initiative?.result_toc_results?.[0]?.initiative_id
      )
      .subscribe(({ response }) => {
        this.theoryOfChangesServices.resultActionArea.push(response);
        this.theoryOfChangesServices.resultActionArea[this.indexList].init =
          this.initiative?.result_toc_results?.initiative_id;
        this.theoryOfChangesServices.resultActionArea[
          this.indexList
        ].consImpactTarget.map(
          item =>
