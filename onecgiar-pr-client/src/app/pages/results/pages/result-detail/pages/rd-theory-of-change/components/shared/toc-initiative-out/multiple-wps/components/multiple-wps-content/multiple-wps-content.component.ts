...
export class MultipleWPsContentComponent implements OnChanges {
    ...
    if (this.showMultipleWPsContent) {
      if (
        (this.resultLevelId === 1 &&
          ...
          this.eoiList.length > 0) ||
        (this.resultLevelId === 2 &&
          this.outcomeList.length > 0 &&
          this.eoiList.length > 0)
      ) {
        if (
          this.activeTab?.toc_result_id &&
          this.activeTab?.initiative_id &&
          !this.activeTab?.indicators?.length
        ) {
          this.getIndicator();
        }
      }
    ...
    this.api.resultsSE
      ...
      .subscribe({
        next: ({ response }) => {
          ...
          this.activeTab.impactAreasTargets = response?.impactAreas.map(
            item => ({
              ...item,
          ...
          this.activeTab.actionAreaOutcome = response?.actionAreaOutcome.map(
            item => ({
              ...item,
              full_name: `${
                item.actionAreaId === 1
          ...
          this.activeTab.wpinformation.wpTitle = response.wpinformation
            ?.extraInformation?.wp_acronym
  ...
  dynamicProgressLabel() {
    if (this.activeTab?.planned_result && this.resultLevelId === 1)
      return `Progress narrative of the Output`;
  ...
  validateSelectedOptionOutPut(tab?: any) {
    const selectedOption = tab
      ...
      : this.outputList.find(
          item => item.toc_result_id === this.activeTab?.toc_result_id
        );
    ...
    this.selectedOptionsOutput = this.selectedOptionsOutput.filter(
      item => item.tabId !== selectedOption.tabId
    );
    ...
    this.outputList = this.outputList.map(item => {
      const finded = this.selectedOptionsOutput.find(
        option =>
          option.tabId !== this.activeTab.uniqueId &&
          option.work_package_id === item.work_package_id
      );
  ...
  validateSelectedOptionEOI(tab?: any) {
    ...
    this.eoiList = this.eoiList.map(item => {
      const finded = this.selectedOptionsEOI.find(
        option => option.toc_result_id === item.toc_result_id
      );
      item.disabledd = !!finded;
      return item;
    });
  ...
  dynamicMappedResultButtonText() {
    return `See all results contributing to this TOC ${
      this.activeTab?.planned_result && this.resultLevelId === 1
        ? 'Output'
        : 'Outcome'
    }`;
  }
  ...
  openMappedResultsModal() {
    ...
    this.mappedResultService.columnsOrder = [
      ...
      { title: 'Phase', attr: 'phase_name' },
      {
        ...
        attr: 'toc_progressive_narrative'
      }
    ];
  }
  ...
  showNarrative(): boolean {
    if (
      this.resultLevelId === 2 ||
      (this.resultLevelId === 1 && !this.activeTab?.planned_result)
    )
      return true;
    ...
    return this.activeTab.indicators.some(indicator => {
      return indicator.targets?.some(
        target => target.indicator_question === false
      );
    });
  }
}
