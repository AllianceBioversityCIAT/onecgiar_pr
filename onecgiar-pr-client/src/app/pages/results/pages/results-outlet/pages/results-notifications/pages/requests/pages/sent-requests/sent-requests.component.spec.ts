...
describe('SentRequestsComponent', () => {
  ...
  it('groups a today row and a three-week-old row under Today/Earlier headers with counts, and drops Pending/Done text', () => {
    ...
    resultsNotificationsServiceMock.sentData = {
      sentContributionsPending: [buildNotification({ share_result_request_id: 1, request_status_id: 1, requested_date: todayIso })],
      sentContributionsDone: [buildNotification({ share_result_request_id: 2, request_status_id: 2, requested_date: threeWeeksAgo })]
    };
    ...
    const headers = fixture.nativeElement.querySelectorAll('h1.sent_container_header_title');
  ...
  it('filters by centerIdsFilter using the NOTIF-T-6-wired filterNotificationByCenter pipe, identically to Received', () => {
    resultsNotificationsServiceMock.sentData = {
      sentContributionsPending: [
        buildNotification({
          share_result_request_id: 1,
  ...
  it('renders the skeleton while loading', () => {
    resultsNotificationsServiceMock.loadingSent = true;
  ...
  it('renders the empty state when there is nothing to show', () => {
    resultsNotificationsServiceMock.loadingSent = false;
    resultsNotificationsServiceMock.sentData = {
      sentContributionsPending: [],
      sentContributionsDone: []
    };
