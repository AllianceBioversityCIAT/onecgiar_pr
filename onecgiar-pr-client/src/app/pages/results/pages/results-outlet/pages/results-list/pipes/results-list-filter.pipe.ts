...
export class ResultsListFilterPipe implements PipeTransform {
  ...
  transform(
    ...
    return this.convertList(
      this.filterByPhase(
        this.filterByResultLevelOptions(
          this.filterByInitsAndYear(this.filterByText(resultList, word))
        )
      ),
      combine
    );
  ...
  filterByInitsAndYear(resultList: any[]) {
    ...
    for (const option of submitter?.options)
      if (option?.selected === true && option?.cleanAll !== true)
        resultsFilters.push(option);
    if (!resultsFilters.length) return resultList;
    resultList = resultList.filter(result => {
      //(result);
      for (const filter of resultsFilters)
        if (
          filter?.id == result?.submitter_id ||
          (filter?.attr == 'is_legacy' && result.legacy_id)
        )
          return true;
      return false;
    });
  ...
  filterByPhase(resultList: any[]) {
    ...
    resultList = resultList.filter(result => {
      //(result);
      for (const filter of resultsFilters)
        if (filter?.attr == result?.phase_name) return true;
      return false;
    });
  ...
  filterByResultLevelOptions(resultList: any[]) {
    ...
    this.resultsListFilterSE.filters.resultLevel.map((filter: any) => {
      for (const option of filter?.options)
        if (option?.selected === true)
          resultsFilters.push({
            result_level_id: filter?.id,
            result_type_id: option?.id
          });
    });
    ...
    resultList = resultList.filter(result => {
      for (const filter of resultsFilters)
        if (
          filter.result_level_id == result.result_level_id &&
          filter.result_type_id == result.result_type_id
        )
          return true;
      return false;
    });
