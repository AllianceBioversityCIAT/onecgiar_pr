...
export class ResultsListComponent implements OnInit, OnDestroy {
  ...
  onPressAction(result) {
    ...
    this.itemsWithDelete[1].visible =
      this.api.dataControlSE.currentResult?.phase_year <
        this.api.dataControlSE.reportingCurrentPhase &&
      this.api.dataControlSE.currentResult?.phase_year !==
        this.api.dataControlSE.reportingCurrentPhase;
  }
  ...
  onDeleteREsult() {
    this.api.alertsFe.show(
      ...
      () => {
        ...
        setTimeout(() => {
          document
            .getElementById('custom-spinner')
            .scrollIntoView({ behavior: 'smooth' });
        }, 100);
        this.api.resultsSE
          .PATCH_DeleteResult(this.api.dataControlSE.currentResult.id)
          .subscribe({
            next: resp => {
              this.api.alertsFe.show({
