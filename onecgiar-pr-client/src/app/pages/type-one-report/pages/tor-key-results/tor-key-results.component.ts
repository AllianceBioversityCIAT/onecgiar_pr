...
@Component({
  ...
  imports: [CommonModule, AlertStatusComponent, PrButtonComponent]
})
export class TorKeyResultsComponent implements OnDestroy {
  ...
  keyResultsDesc =
    name => `This section provides an overview of results reported by the CGIAR Initiative on <strong>${name}</strong>. These results align with the CGIAR Results Framework and <strong>${name}</strong> theory of change.
  ...
  exportExcel(initiativeSelected) {
    ...
    this.api.resultsSE
      .GET_excelFullReportByInitiativeId(
        this.typeOneReportSE.getInitiativeID(initiativeSelected)?.id,
      ...
      .subscribe({
        next: ({ response }) => {
          this.exportTablesSE.exportExcel(
            response,
            'Initiative-progress-and-key-results'
          );
