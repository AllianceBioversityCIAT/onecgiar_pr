...
export class MassivePhaseShiftComponent implements OnInit {
  ...
  accept() {
    ...
    this.api.resultsSE.PATCH_versioningAnnually().subscribe({
      next: (resp: any) => {
        ...
        this.api.alertsFe.show({
          ...
          description:
            ...
            ' results of type Innovation Development have been replicated from the previous phase to the current phase.',
          status: 'success'
        });
        this.GET_numberOfResultsByResultType();
      },
      error: err => {
        this.api.alertsFe.show({
          id: 'PATCH_versioningAnnually',
          title: 'Process execution failed',
          description: err?.error?.meesage,
          status: 'error',
          closeIn: 500
        });
        this.api.dataControlSE.massivePhaseShiftIsRunning = false;
