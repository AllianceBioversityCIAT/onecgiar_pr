...
// `phase`, `status`, and `createdBy` are MULTI-VALUE: comma-separated lists (`?phase=A,B`,
// `?status=Editing,Submitted`, `?createdBy=Ana,Ruiz`), encoded by `joinListParam` and decoded by
// `parseListParam` (`programme-results-filter.service.ts`). A SINGLE value stays valid and hydrates
// as a one-element array — legacy deep links keep working unchanged.
// @akili-spec changes/my-work-board (MWB-T-13)
// The next three share the same shape. Both hosts (Results tab and My results board) read and write
// them this way; Overview → Results deep links still emit one `category`/`origin`/`center`.
