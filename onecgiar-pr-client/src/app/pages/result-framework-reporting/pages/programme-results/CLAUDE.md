...
- Row activation mirrors `results-list.component.ts` (`getResultRoute()` / `navigateToResult()`) exactly: a `W3/Bilaterals` row that is
  neither AVISA (`SGP-02`) nor Editing/Draft/`Approved` opens the bilateral **review drawer**; Editing/Draft/`Approved` with a lead center opens the center editor (`/bilateral/<center>/result/<code>?phase=`; spec `bugfix/bilateral-approved-open-route`), without one it falls back to Result Detail (BAO-R-3); everything else
  Result Detail with `?phase=versionId` (`resultRoute()`, `:1515`).
