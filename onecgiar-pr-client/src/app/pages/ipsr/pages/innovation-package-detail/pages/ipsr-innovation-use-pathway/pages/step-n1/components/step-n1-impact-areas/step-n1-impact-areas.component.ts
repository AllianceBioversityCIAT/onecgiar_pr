...
export class StepN1ImpactAreasComponent implements OnInit {
  ...
  GET_AllClarisaImpactAreaIndicators() {
    this.api.resultsSE.GET_AllglobalTarget().subscribe(({ response }) => {
      ...
      this.allImpactAreaIndicators.forEach(
        item =>
  ...
  filterImpactAreaIndicatorsByImpactAreaID(iaID) {
    return this.allImpactAreaIndicators.filter(
      (item: any) => item?.impactAreaId == iaID
    );
  }

  removeOption(option) {
    const index = this.body.impactAreas.findIndex(
      (valueItem: any) => valueItem.targetId == option.targetId
    );
