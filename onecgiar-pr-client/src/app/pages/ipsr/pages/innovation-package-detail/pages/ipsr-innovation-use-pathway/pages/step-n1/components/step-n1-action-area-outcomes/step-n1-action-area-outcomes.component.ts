...
export class StepN1ActionAreaOutcomesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.api.resultsSE.GETAllClarisaActionAreasOutcomes().subscribe({
      next: ({ response }) => {
        ...
        this.actionAreasOutcomesList.geneticInnovation.map(
          item =>
            (item.full_name = `<strong>${item.outcomeSMOcode}</strong> - ${item.outcomeStatement}`)
        );
        this.actionAreasOutcomesList.resilientAgrifoodSystems.map(
          item =>
            (item.full_name = `<strong>${item.outcomeSMOcode}</strong> - ${item.outcomeStatement}`)
        );
        this.actionAreasOutcomesList.systemTrasnformation.map(
          item =>
            (item.full_name = `<strong>${item.outcomeSMOcode}</strong> - ${item.outcomeStatement}`)
        );
      },
  ...
  removeOption(option) {
    const index = this.body.actionAreaOutcomes.findIndex(
      valueItem =>
        valueItem.action_area_outcome_id == option.action_area_outcome_id
    );
    this.body.actionAreaOutcomes.splice(index, 1);
  }

  filterByAAOId(id) {
    return this.body.actionAreaOutcomes.filter(
      (item: any) => item?.actionAreaId == id
    );
  }
}
