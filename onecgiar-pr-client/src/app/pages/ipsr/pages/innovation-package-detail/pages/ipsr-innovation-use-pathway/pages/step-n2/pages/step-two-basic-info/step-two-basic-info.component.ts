    ...
    this.api.resultsSE.GETInnovationPathwayStepTwoInnovationSelect().subscribe(resp => {
      ...
      if (this.informartion.length != 0) {
        ...
        this.informartion.map(async elem => {
          const aux: innovationComplementary = new innovationComplementary();
  ...
  selectedOneLevel(category, i, levels) {
    if ((levels = 1)) {
      if (category.subCategories.length != 0) {
        const index = this.bodyStep22[i].complementary_innovation_enabler_types_one.findIndex(ele => ele.complementary_innovation_enabler_types_id == category.complementary_innovation_enabler_types_id);
        //(index);
        if (index != -1) {
          ...
          category.subCategories.forEach(element => {
            const aux = this.bodyStep22[i].complementary_innovation_enabler_types_two.findIndex(ele => ele.complementary_innovation_enabler_types_id == element.complementary_innovation_enabler_types_id);
            //(aux);
            if (aux != -1) {
