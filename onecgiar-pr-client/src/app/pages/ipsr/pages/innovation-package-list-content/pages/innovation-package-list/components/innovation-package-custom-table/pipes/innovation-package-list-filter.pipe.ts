...
export class InnovationPackageListFilterPipe implements PipeTransform {
  ...
  transform(list, word: string, initsSelectedJoinText: number) {
    return this.filterByInits(
      this.filterByText(this.filterByPhase(list), word)
    );
  }
  ...
  filterByInits(list) {
    if (
      this.ipsrListFilterSE.filters.general[0].options.every(
        init => init.selected !== true || init.cleanAll
      )
    )
      return list;
    return list.filter(item => {
      return Boolean(
        this.ipsrListFilterSE.filters.general[0].options.find(
          init => init.attr == item?.official_code && init.selected
        )
      );
    });
  }
  ...
  filterByPhase(list) {
    ...
    for (const option of this.ipsrListFilterSE.filters.general[1].options)
      if (option?.selected === true) resultsFilters.push(option);
