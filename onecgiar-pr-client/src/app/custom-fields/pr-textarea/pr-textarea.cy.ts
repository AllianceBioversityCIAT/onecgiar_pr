...
/**
 * Behavior lock for <app-pr-textarea>.
 * Captures: two-way ngModel, external value reflection, read-only view, word-count invalid state.
 *
 * NOTE (Spartan migration): PrimeNG was removed on `performance-refactor`, so the control is no
 * longer `textarea[pTextarea]` — it is a plain `<textarea>` inside the `.pr-field` root. Only the
 * SELECTORS were updated here; the assertions are the same behaviour this spec always locked.
 */
describe('PrTextareaComponent (CT)', () => {
  /** The editable control rendered by pr-textarea. */
  ...
  it('typing updates the bound model', () => {
    mountCF(TEMPLATE, { editable: true, componentProperties: { model: '', maxWords: null, readOnly: false } }).then(w => {
      cy.get(TEXTAREA).should('be.visible').type('some notes here');
  ...
  it('reflects an external value change', () => {
    mountCF(TEMPLATE, { editable: true, componentProperties: { model: 'first', maxWords: null, readOnly: false } }).then(w => {
      ...
      cy.get(TEXTAREA).should('have.value', 'changed outside');
    });
  });
