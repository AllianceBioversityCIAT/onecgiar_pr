...
export const routingApp: PrRoute[] = [
  ...
  { prName: 'Type 1 report elements', underConstruction: false, onlytest: false, canActivate: [CheckLoginGuard], path: 'type-one-report', loadChildren: () => import('../../pages/type-one-report/type-one-report.module').then(m => m.TypeOneReportModule) },
  { prName: 'Innovation Packages', underConstruction: false, onlytest: true, canActivate: [CheckLoginGuard], path: 'ipsr', loadChildren: () => import('../../pages/ipsr/ipsr.module').then(m => m.IpsrModule) },
  ...
  { prName: 'reports', prHide: true, path: 'reports/fullResultReport/:id', loadChildren: () => import('../../pages/pdf-reports/pdf-reports.module').then(m => m.PdfReportsModule) },
