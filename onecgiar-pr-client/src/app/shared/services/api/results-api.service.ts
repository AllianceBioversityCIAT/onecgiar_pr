    ...
    return this.http.post<any>(`${this.apiBaseUrl}admin-panel/report/results/completeness`, { rol_user, initiatives }).pipe(
      map(resp => {
        ...
        resp?.response.map(result => {
          ...
          result.full_name_html = `<strong>Result code: (${result.result_code})</strong> - ${result.result_title}  - <strong>Official code: (${result.official_code})</strong> - <strong>Result Type: (${result.result_type_name})</strong>`;
          result.result_code = Number(result.result_code);
